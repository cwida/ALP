 ...
 * API Compressor
 */
template <class PT, bool DRY = false>
struct AlpCompressor {
	...
	PT       input_vector[config::VECTOR_SIZE];
	PT       exceptions[config::VECTOR_SIZE];
	PT       sample_array[config::VECTOR_SIZE];
	...
	void compress_alp_vector() {
		...
		encoder<PT>::analyze_ffor(encoded_integers, stt.bit_width, &stt.for_base);
	...
	void compress(PT* values, size_t values_count, uint8_t* out) {
		...
		for (size_t current_rowgroup = 0; current_rowgroup < rouwgroup_count; current_rowgroup++) {
			 ...
			 * Rowgroup level
			 */
			encoder<PT>::init(values, current_idx, values_count, sample_array, stt);
			if (stt.scheme == Scheme::ALP_RD) {
				rd_encoder<PT>::init(values, current_idx, values_count, sample_array, stt);
		...
		if (left_to_compress) { // Last vector which may be incomplete
			...
			if (stt.scheme == Scheme::ALP_RD) {
				AlpApiUtils<PT>::fill_incomplete_alprd_vector(input_vector, stt);
			} else {
				AlpApiUtils<PT>::fill_incomplete_alp_vector(
				    input_vector, exceptions, exceptions_position, &stt.exceptions_count, encoded_integers, stt);
			}
			compress_vector();
			store_vector();
	...
	void compress_rd(PT* values, size_t values_count, uint8_t* out) {
		storer                  = storer::MemStorer<DRY>(out);
		size_t rouwgroup_count  = AlpApiUtils<PT>::get_rowgroup_count(values_count);
		size_t current_idx      = 0;
		size_t left_to_compress = values_count;
	...
	void store_alp_vector() {
		...
		if (stt.exceptions_count) {
			storer.store((void*)exceptions, Constants<PT>::EXCEPTION_SIZE_BYTES * stt.exceptions_count);
