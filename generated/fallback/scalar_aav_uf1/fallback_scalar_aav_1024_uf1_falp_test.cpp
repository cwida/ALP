...
class fallback_scalar_aav_1024_uf1_falp : public ::testing::Test {
public:
	double*   dbl_arr;
	double*   exc_arr;
	uint16_t* pos_arr;
	...
	~fallback_scalar_aav_1024_uf1_falp() override {
		delete[] dbl_arr;
		delete[] exc_arr;
		delete[] pos_arr;
		delete[] dig_arr;
...
TEST_F(fallback_scalar_aav_1024_uf1_falp, fused) {
...
TEST_F(fallback_scalar_aav_1024_uf1_falp, unfused) {
	for (auto& dataset : alp_bench::alp_dataset) {
		std::ifstream ifile(dataset.sample_csv_file_path, std::ios::in);
		ASSERT_EQ(ifile.fail(), false);
		alp::state stt;
