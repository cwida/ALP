...
static __attribute__((noinline)) benchmark::BenchmarkReporter::Run b_a_e(const double*      dbl_arr,
	...
	return benchmark::BenchmarkReporter::Run(
	    benchmark_number, benchmark_name, iterations, double(cycles) / (double(iterations) * VECTOR_SIZE));
}
...
void benchmark_all(benchmark::Benchmark& benchmark) {
	...
	dbl_arr          = new (std::align_val_t {64}) double[VECTOR_SIZE];
	exc_arr          = new (std::align_val_t {64}) uint16_t[VECTOR_SIZE];
	pos_arr          = new (std::align_val_t {64}) uint16_t[VECTOR_SIZE];
	encoded_arr      = new (std::align_val_t {64}) int64_t[VECTOR_SIZE];
	dec_dbl_arr      = new (std::align_val_t {64}) double[VECTOR_SIZE];
	exc_c_arr        = new (std::align_val_t {64}) uint16_t[VECTOR_SIZE];
	ffor_arr         = new (std::align_val_t {64}) int64_t[VECTOR_SIZE];
	base_arr         = new (std::align_val_t {64}) int64_t[VECTOR_SIZE];
...
int main() {
	benchmark::Benchmark benchmark =
	    benchmark::create("alp_encode_cutter")
	        .save()
