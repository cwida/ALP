...
static bool put_object(const Aws::S3Crt::S3CrtClient &s3_client, const std::string &bucket, std::shared_ptr<std::stringstream> &sstream, long object_idx, long object_size) {
...
static void generate_data(std::shared_ptr<std::stringstream> &sstream, long object_size) {
    ...
    thread_local std::uniform_int_distribution<int> dist {std::numeric_limits<char>::min(), std::numeric_limits<char>::max()};
...
static std::pair<bool, std::string> upload_part(const Aws::S3Crt::S3CrtClient &s3_client, const std::string &bucket, std::string &key, std::string &upload_id, size_t part_number, std::shared_ptr<std::stringstream> &sstream) {
...
static void generate_and_upload_multipart(const Aws::S3Crt::S3CrtClient &s3_client, const std::string &bucket, long number_of_objects, long object_size) {
    tbb::parallel_for(long(0), number_of_objects, [&](long object_idx) {
        ...
        tbb::parallel_for(size_t(1), num_parts+1, [&](size_t part_number){
            ...
            if (!success) {
                ...
                while(!cleanup_done) {
                    std::cerr << "Uploading part " << part_number << " for id " << upload_id << " failed. Attempting abort.." << std::endl;
                    ...
                    // List parts to make sure there is nothing left. The docs says we have to do this to ensure proper cleanup.
...
static void generate_and_upload(const Aws::S3Crt::S3CrtClient &s3_client, const std::string &bucket, long number_of_objects, long object_size) {
    tbb::parallel_for(long(0), number_of_objects, [&](long object_idx) {
        auto sstream = std::make_shared<std::stringstream>();
