...
DEFINE_bool(sampling_test_mode, true, "we are in sampling test mode, trying all schemes.");
...
struct RandomSampler : Sampler<T> {
  ...
  RandomSampler() : RandomSampler(FLAGS_sample_size, FLAGS_sample_count) {}
  ...
  sample_t<T> operator()([[maybe_unused]] const T* input, [[maybe_unused]] size_t count, stats_t& stats) const override {
    FLAGS_sample_count = this->sample_count;
    FLAGS_sample_size = this->sample_size;
...
TestResult testSampling(const std::string& filename, const std::string& bitmap_file, u8* output, PerfEvent& e) {
    ...
    //std::vector<std::pair<int, int>> combinations = {
    //{10, 8}, {10, 16}, {10, 32}, {10, 64}, {10, 128}, {10, 256}, {10, 512}, {10, 1024},
    //{50, 8}, {50, 16}, {50, 32}, {50, 64}, {50, 128}, {50, 256}, {50, 512}, {50, 5024},
    //};
    std::vector<std::pair<int,int>> combinations;
    // growing size
    //auto sample_size = 64u;
    //for (auto sample_count : {1,2,5,10,20,50,100,500,1000}) {
    //    combinations.push_back({sample_count, sample_size});
    //}
    //// growing count
    auto sample_count = 10;
    for (auto sample_size : {1,2,4,8,16,32,64,128,256,512,1024,2048,4096}) {
      combinations.push_back({sample_count, sample_size});
    }

    //for (auto sample_count = 1u; sample_count < (65535/sample_size)/2; sample_count *= 2) {
