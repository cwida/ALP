...
public:
   ...
   tuple<vector<T>, vector<BITMAP>> samples(u32 n, u32 length)
   {
      ...
      // -------------------------------------------------------------------------------------
      // TODO : Construction Site !! need a better theory and algorithm for sampling
      // Constraints: RLE(runs), nulls, uniqueness,
      // naive approach to pick enough elements that approximate stratified sample
      // run ~8 normal rounds, then check if the
      ...
      if ( tuple_count <= n * length ) {
         ...
         u32 separator = tuple_count / n; // how big is the slice of the input, of which we take a part....
         ...
         for ( u32 sample_i = 0; sample_i < n; sample_i++ ) {
            ...
            // (sample_i * separator, (sample_i + 1 ) * separator) range to pick from
            compiled_values.insert(compiled_values.end(), src + partition_begin, src + partition_begin + length);
            if ( bitmap == nullptr ) {
               compiled_bitmap.insert(compiled_bitmap.end(), length, 1);
            } else {
               compiled_bitmap.insert(compiled_bitmap.end(), bitmap + partition_begin, bitmap + partition_begin + length);
            }
         }
      }
