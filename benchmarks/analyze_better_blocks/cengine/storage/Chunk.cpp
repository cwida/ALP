...
// -------------------------------------------------------------------------------------
    bool Chunk::operator==(const cengine::Chunk &other) const {
        ...
        for (u32 column_i = 0; column_i < relation.columns.size(); column_i++) {
            if (!column_requires_copy[column_i] && sizes[column_i] != other.sizes[column_i]) {
                cerr << "== : sizes in column " << column_i << " are not identical: " << sizes[column_i] << " vs. "
                     << other.sizes[column_i] << endl;
                return false;
            }

            if ((bitmaps[column_i] && other.bitmaps[column_i]) &&
                (std::memcmp(bitmaps[column_i].get(), other.bitmaps[column_i].get(), sizeof(BITMAP) * tuple_count) !=
                 0)) {
                ...
                if (bitmaps[column_i][row_i]) {
                    switch (relation.columns[column_i].type) {
                        case ColumnType::INTEGER: {
                            ...
                            if (me != they) {
                                cerr << "== : INTEGER column (" << relation.columns[column_i].name
                                     << ") data are not identical\t" << "row_i = " << row_i << endl << me << endl
                                     << they << endl;
                                return false;
                            }
                            break;
                        ...
                        case ColumnType::STRING: {
                            ...
                            if (me.length() != they.length() || std::memcmp(me.data(), they.data(), me.length()) != 0) {
                                cerr << "== : STRING column (" << relation.columns[column_i].name
                                     << ") data are not identical\t" << "row_i = " << row_i << endl;
...
// -------------------------------------------------------------------------------------
    Chunk::Chunk(unique_ptr<unique_ptr<u8[]>[]> &&columns, unique_ptr<unique_ptr<BITMAP[]>[]> &&bitmaps,
                 unique_ptr<bool[]> &&column_requires_copy, u64 tuple_count, const Relation &relation,
                 unique_ptr<SIZE[]> &&sizes)
            : relation(relation), columns(std::move(columns)), bitmaps(std::move(bitmaps)),
              column_requires_copy(std::move(column_requires_copy)), sizes(std::move(sizes)),
              tuple_count(tuple_count) {}

    Chunk::Chunk(unique_ptr<unique_ptr<u8[]>[]> &&columns, unique_ptr<unique_ptr<BITMAP[]>[]> &&bitmaps,
                 u64 tuple_count, const Relation &relation, unique_ptr<SIZE[]> &&sizes)
            : relation(relation), columns(std::move(columns)), bitmaps(std::move(bitmaps)), sizes(std::move(sizes)),
              tuple_count(tuple_count) {
...
// -------------------------------------------------------------------------------------
    InputChunk::InputChunk(unique_ptr<u8[]> &&data, unique_ptr<BITMAP[]> &&bitmap, ColumnType type, u64 tuple_count,
                           SIZE size)
            : data(std::move(data)),
    ...
    bool InputChunk::compareContents(u8 *their_data, BitmapWrapper *bitmapWrapper, u64 their_tuple_count,
                                     bool requires_copy) const {
        if (their_tuple_count != this->tuple_count) {
            std::cerr << "Tuple count is not equal. Expected: " << this->tuple_count << ". Got: " << their_tuple_count << std::endl;
            return false;
        }

        std::vector<BITMAP> their_bitmap(their_tuple_count);
        bitmapWrapper->writeBITMAP(their_bitmap.data());
        ...
        switch (this->type) {
                ...
                for (u64 idx = 0; idx < their_tuple_count; idx++) {
                    if (this->nullmap[idx] && my_doubles[idx] != their_doubles[idx]) {
                        std::cerr << "Double data is not equal at index " << idx << std::setprecision(1000)
                                  << " Expected: " << my_doubles[idx]
                                  << " Got: " << their_doubles[idx] << std::endl;
                        return false;
                    }
                }
                break;
            ...
            case ColumnType::STRING: {
                auto my_view = cengine::StringArrayViewer(this->data.get());
                for (u64 idx = 0; idx < their_tuple_count; idx++) {
                    if (!this->nullmap[idx]) {
                        continue;
                    }
                    ...
                    str their_str;
                    if (requires_copy) {
                        auto their_view = cengine::StringPointerArrayViewer(their_data);
                        their_str = their_view(idx);
                    } else {
                        auto their_view = cengine::StringArrayViewer(their_data);
                        their_str = their_view(idx);
                    }

                    if (my_str.length() != their_str.length()) {
                        std::cerr << "String lengths are not equal at index " << idx << " Expected: " << my_str << " "
                                  << my_str.length() << " Got: " << their_str << " " << their_str.length() << std::endl;
                        return false;
                    }

                    if (my_str != their_str) {
                        std::cerr << "Strings are not equal at index " << idx << " Expected: " << my_str << " Got: "
                                  << their_str << std::endl;
                        return false;
                    }
                }
    ...
    bool ColumnPart::canAdd(SIZE chunk_size) {
        if (chunk_size > FLAGS_part_size_threshold) {
            ...
            // Although writing will work, reading the data back in may break, as we assume objects to always have a
            // maximum size of part_size_threshold.
            throw std::logic_error("chunks with compressed size greater than part_size_threshold unsupported");
        }
