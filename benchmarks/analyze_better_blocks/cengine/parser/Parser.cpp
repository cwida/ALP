...
// -------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------
DEFINE_string(csv_separator, "|", "CSV separator.");
// -------------------------------------------------------------------------------------
namespace cengine {
// -------------------------------------------------------------------------------------
void Parser::parse(const string csv_path, const YAML::Node &schema, const string &out_dir)
{
   ...
   // read csv data and fill the columns
   {
      ...
      aria::csv::CsvParser parser = aria::csv::CsvParser(csv_stream)
        .delimiter(*FLAGS_csv_separator.c_str())
        .terminator('\n');
            ...
            for ( auto &column_csv_str: tuple ) {
               ...
               switch ( column_descriptor.column_type ) {
                  ...
                  case ColumnType::DOUBLE: {
                     ...
                     column_descriptor.set_bitmap.push_back(is_set);
                     // -------------------------------------------------------------------------------------
                     DOUBLE value = (is_set ? std::stod(column_str) : NULL_CODE);
