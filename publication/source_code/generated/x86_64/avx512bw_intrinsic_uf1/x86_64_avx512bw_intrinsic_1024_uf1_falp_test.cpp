...
#include <fstream>
class x86_64_avx512bw_intrinsic_1024_uf1_falp : public ::testing::Test {
public:
	double*   dbl_arr;
	double*   exc_arr;
	uint16_t* pos_arr;
	...
	~x86_64_avx512bw_intrinsic_1024_uf1_falp() override {
		delete[] dbl_arr;
		delete[] exc_arr;
		delete[] pos_arr;
		delete[] dig_arr;
...
TEST_F(x86_64_avx512bw_intrinsic_1024_uf1_falp, fused) {
...
TEST_F(x86_64_avx512bw_intrinsic_1024_uf1_falp, unfused) {
	for (auto& dataset : alp_bench::get_alp_dataset()) {
		std::ifstream ifile(dataset.csv_file_path, std::ios::in);
		ASSERT_EQ(ifile.fail(), false);
		alp::state<double> stt;
