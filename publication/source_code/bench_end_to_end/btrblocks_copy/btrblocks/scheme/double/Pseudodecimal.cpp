 ...
 */
// -------------------------------------------------------------------------------------
u32 Decimal::compress(const DOUBLE *src, const BITMAP *, u8 *dest,
                      DoubleStats &stats, u8 allowed_cascading_level) {
    ...
    for (u32 row_i = row_start_i; row_i < row_end_i; row_i++) {
      ...
      if (current_double == -0.0 && std::signbit(current_double)) {
        ...
        for (exponent = 0; exponent <= max_exponent; exponent++) {
          ...
          DOUBLE if_converted_back = static_cast<DOUBLE>(converted_number) *
                                     exact_fractions_of_ten[exponent];
          if (if_converted_back == current_double &&
              ((std::floor(std::log2(converted_number)) + 1) <=
               significant_digits_bits_limit)) {
            convertable = true;
            break;
          }
        }
      ...
      if (convertable) {
        ...
        //                    // This is a hacky way to avoid using Decimal in
        //                    columns where there are many exceptions
        //                    // Return a big number will make the selection
  ...
  if (!numbers_v.empty()) {
    u32 used_space;
    IntegerSchemePicker::compress(numbers_v.data(), nullptr, write_ptr,
                                  numbers_v.size(), allowed_cascading_level - 1,
                                  used_space, col_struct.numbers_scheme,
                                  autoScheme(), "significant digits");
    write_ptr += used_space;
    Log::debug("Decimal: sd_c = {} sd_s = {}", CI(col_struct.numbers_scheme),
               CI(used_space));
  }
  ...
  // Compress exponents
  {
    ...
    u32 used_space;
    SInteger32Stats e_stats = SInteger32Stats::generateStats(
        exponent_v.data(), nullptr, exponent_v.size());
    // cout << e_stats.min << '\t' << e_stats.max << endl;
    IntegerSchemePicker::compress(
        exponent_v.data(), nullptr, write_ptr, exponent_v.size(),
        allowed_cascading_level - 1, used_space, col_struct.exponents_scheme,
        autoScheme(), "exponents");
    write_ptr += used_space;
    Log::debug("Decimal: e_c = {} e_s = {}", CI(col_struct.exponents_scheme),
               CI(used_space));
  }
  ...
  // Compress patches
  {
    ...
    u32 used_space;
    DoubleSchemePicker::compress(patches_v.data(), nullptr, write_ptr,
                                 patches_v.size(), allowed_cascading_level - 1,
                                 used_space, col_struct.patches_scheme,
                                 autoScheme(), "patches");
    write_ptr += used_space;
    Log::debug("Decimal: p_c = {} p_s = {}", CI(col_struct.patches_scheme),
               CI(used_space));
  }
  ...
  // Write exceptions bitmap
  {
    ...
    write_ptr +=
        exceptions_bitmap.write(reinterpret_cast<char *>(write_ptr), false);
  }
...
static inline void decompressAVXBlock4(DecimalIterateParam *param) {
  ...
  __m128i numbers_int_0 =
      _mm_loadu_si128(reinterpret_cast<__m128i *>(param->numbers_ptr) + 0);
  __m128i numbers_int_1 =
      _mm_loadu_si128(reinterpret_cast<__m128i *>(param->numbers_ptr) + 1);
  __m128i numbers_int_2 =
      _mm_loadu_si128(reinterpret_cast<__m128i *>(param->numbers_ptr) + 2);
  __m128i numbers_int_3 =
      _mm_loadu_si128(reinterpret_cast<__m128i *>(param->numbers_ptr) + 3);
  ...
  __m128i exponents_0 =
      _mm_loadu_si128(reinterpret_cast<__m128i *>(param->exponents_ptr) + 0);
  __m128i exponents_1 =
      _mm_loadu_si128(reinterpret_cast<__m128i *>(param->exponents_ptr) + 1);
  __m128i exponents_2 =
      _mm_loadu_si128(reinterpret_cast<__m128i *>(param->exponents_ptr) + 2);
  __m128i exponents_3 =
      _mm_loadu_si128(reinterpret_cast<__m128i *>(param->exponents_ptr) + 3);

  // Gather powers
  __m256d powers_0 =
      _mm256_i32gather_pd(exact_fractions_of_ten, exponents_0, 8);
  __m256d powers_1 =
      _mm256_i32gather_pd(exact_fractions_of_ten, exponents_1, 8);
  __m256d powers_2 =
      _mm256_i32gather_pd(exact_fractions_of_ten, exponents_2, 8);
  __m256d powers_3 =
      _mm256_i32gather_pd(exact_fractions_of_ten, exponents_3, 8);
...
static inline void decompressAVXBlock1(DecimalIterateParam *param) {
  // Load numbers and convert to double
  __m128i numbers_int =
      _mm_loadu_si128(reinterpret_cast<__m128i *>(param->numbers_ptr));
  ...
  __m256d powers =
      _mm256_set_pd(exact_fractions_of_ten[param->exponents_ptr[3]],
...
static inline void decompressAVXBlockUnroll(DecimalIterateParam *param,
                                            uint32_t limit) {
#if 1
...
static inline void decompressAVXBlock(DecimalIterateParam *param,
                                      uint32_t limit) {
  while (param->next_block_i < limit) {
    decompressAVXBlock1(param);
  }
...
void Decimal::decompress(DOUBLE *dest, BitmapWrapper *, const u8 *src,
                         u32 tuple_count, u32 level) {
  ...
  auto numbers_ptr = get_level_data(
      numbers_v, col_struct.converted_count + SIMD_EXTRA_ELEMENTS(INTEGER),
      level);
  thread_local std::vector<std::vector<INTEGER>> exponents_v;
  auto exponents_ptr = get_level_data(
      exponents_v, tuple_count + SIMD_EXTRA_ELEMENTS(INTEGER), level);
  thread_local std::vector<std::vector<DOUBLE>> patches_v;
  auto patches_ptr = get_level_data(patches_v,
                                    tuple_count - col_struct.converted_count +
                                        SIMD_EXTRA_ELEMENTS(DOUBLE),
                                    level);
  Roaring exceptions_bitmap =
      Roaring::read(reinterpret_cast<const char *>(
                        col_struct.data + col_struct.exceptions_map_offset),
                    false);

  if (col_struct.converted_count > 0) {
    IntegerScheme &numbers_scheme =
        IntegerSchemePicker::MyTypeWrapper::getScheme(
            col_struct.numbers_scheme);
  ...
  IntegerScheme &exponents_scheme =
      IntegerSchemePicker::MyTypeWrapper::getScheme(
          col_struct.exponents_scheme);
  exponents_scheme.decompress(exponents_v[level].data(), nullptr,
                              col_struct.data + col_struct.exponents_offset,
                              tuple_count, level + 1);
  ...
  if (col_struct.variant_selector & do_iteration) {
    ...
    const u32 num_avx_blocks =
        tuple_count /
        block_size; // The number of blocks that are complete (have 4 values)
    ...
    if (col_struct.variant_selector & do_unroll) {
      exceptions_bitmap.iterate(
          [](uint32_t value, void *param_void) {
            auto param =
                reinterpret_cast<struct DecimalIterateParam *>(param_void);
...
string Decimal::fullDescription(const u8 *src) {
  ...
  if (col_struct.converted_count > 0) {
    IntegerScheme &sd_scheme = IntegerSchemePicker::MyTypeWrapper::getScheme(
        col_struct.numbers_scheme);
    result += "\n\t-> ([int] significant digits) " +
              sd_scheme.fullDescription(col_struct.data);
  }

  IntegerScheme &e_scheme = IntegerSchemePicker::MyTypeWrapper::getScheme(
      col_struct.exponents_scheme);
  result +=
      "\n\t-> ([int] exponents) " +
      e_scheme.fullDescription(col_struct.data + col_struct.exponents_offset);

  DoubleScheme &p_scheme =
      DoubleSchemePicker::MyTypeWrapper::getScheme(col_struct.patches_scheme);
  result +=
      "\n\t-> ([double] patches) " +
...
bool Decimal::isUsable(DoubleStats &stats) {
  double unique_ratio = static_cast<double>(stats.unique_count) /
                        static_cast<double>(stats.tuple_count);
  if (unique_ratio < 0.1) {
    return false;
  }
  return true;
