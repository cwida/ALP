...
QueryBuilder::ResultBuilder& QueryBuilder::ResultBuilder::addValue(std::string name, DS buffer) {
	barrier([&]() { resultWriter.shared.result->result->addAttribute(name, buffer.data_size); });
	...
	resultWriter.inputs.emplace_back(buffer.data, buffer.data_size, attr);
...
void QueryBuilder::DebugCounter(std::string message) {
	...
	Debug<Counter>([](size_t n, Counter& c) { c.counter += n; },
...
QueryBuilder::ProjectionBuilder&
QueryBuilder::ProjectionBuilder::addExpression(std::unique_ptr<class Expression>&& exp) {
...
QueryBuilder::DS QueryBuilder::Buffer(size_t nr, size_t entrySize) {
	if (buffers.find(nr) == buffers.end()) buffers.emplace(nr, make_pair(entrySize, vecs.get(entrySize)));
	return Buffer(nr);
}

QueryBuilder::DS QueryBuilder::Buffer(size_t nr) {
	if (buffers.find(nr) == buffers.end()) throw runtime_error("Buffer " + to_string(nr) + " not found");
	DS r;
	r.buf       = DataStorage::BufferSpec::BUFFER;
	auto& buf   = buffers[nr];
	r.data_size = buf.first;
	r.data      = buf.second;
	return r;
}

QueryBuilder::DS QueryBuilder::Column(ScanBuilder& scan_bdr, std::string attribute) {
	DS r;
	r.buf       = DataStorage::BufferSpec::COLUMN;
	auto& attr  = scan_bdr.rel[attribute];
	r.data_size = attr.type->rt_size();
	r.data      = attr.data();
	r.scan      = &scan_bdr.scan;
	return r;
}
...
void QueryBuilder::pushOperator(std::unique_ptr<Operator>&& op) { operatorStack.push(move(op)); }
...
void QueryBuilder::DataStorage::registerDS(void** location) {
	if (buf == DataStorage::BufferSpec::COLUMN) {
		...
		if (cfg::is_bitpacked) {
			// bitpacked
			scan->addConsumer(location, cfg::bw);
		} else {
			// none bitpacked
			scan->addConsumer(location, data_size);
		}
	}

	if (buf == DataStorage::BufferSpec::MTD) { scan->addConsumer(location, cfg::m_sz); }
}

void QueryBuilder::DataStorage::registerDS(pos_t** location) { registerDS(reinterpret_cast<void**>(location)); }

QueryBuilder::DataStorage::operator void*() const { return data; }
QueryBuilder::DataStorage::operator pos_t*() const { return reinterpret_cast<pos_t*>(data); }

QueryBuilder::ExpressionBuilder& QueryBuilder::ExpressionBuilder::addOp(primitives::F1 op, DS a) {
...
QueryBuilder::ExpressionBuilder& QueryBuilder::ExpressionBuilder::addOp(primitives::F2 op, DS a, DS b) {
...
QueryBuilder::ExpressionBuilder& QueryBuilder::ExpressionBuilder::addOp(primitives::F3 op, DS a, DS b, DS c) {
...
QueryBuilder::ExpressionBuilder& QueryBuilder::ExpressionBuilder::addOp(primitives::F4 op, DS a, DS b, DS c, DS d) {
...
QueryBuilder::ExpressionBuilder::addOp(primitives::F5 op, DS a, DS b, DS c, DS d, DS e) {
	auto f5 = make_unique<F5_Op>(a, b, c, d, e, op);
	a.registerDS(&f5->data_0);
	b.registerDS(&f5->data_1);
	c.registerDS(&f5->data_2);
	d.registerDS(&f5->data_3);
	e.registerDS(&f5->data_4);
	expression->ops.push_back(move(f5));
	return *this;
}

QueryBuilder::ExpressionBuilder&
QueryBuilder::ExpressionBuilder::addOp(primitives::F6 op, DS a, DS b, DS c, DS d, DS e, DS f) {
	auto f6 = make_unique<F6_Op>(a, b, c, d, e, f, op);
	a.registerDS(&f6->data_0);
	b.registerDS(&f6->data_1);
	c.registerDS(&f6->data_2);
	d.registerDS(&f6->data_3);
	e.registerDS(&f6->data_4);
	f.registerDS(&f6->data_5);

	expression->ops.push_back(move(f6));
	return *this;
}

QueryBuilder::ExpressionBuilder&
QueryBuilder::ExpressionBuilder::addOp(primitives::F7 op, DS a, DS b, DS c, DS d, DS e, DS f, DS g) {
	auto f7 = make_unique<F7_Op>(a, b, c, d, e, f, g, op);
	a.registerDS(&f7->data_0);
	b.registerDS(&f7->data_1);
	c.registerDS(&f7->data_2);
	d.registerDS(&f7->data_3);
	e.registerDS(&f7->data_4);
	f.registerDS(&f7->data_5);
	g.registerDS(&f7->data_6);

	expression->ops.push_back(move(f7));
	return *this;
}

QueryBuilder::ExpressionBuilder&
QueryBuilder::ExpressionBuilder::addOp(primitives::F8 op, DS a, DS b, DS c, DS d, DS e, DS f, DS g, DS h) {
	auto f8 = make_unique<F8_Op>(a, b, c, d, e, f, g, h, op);
	a.registerDS(&f8->data_0);
	b.registerDS(&f8->data_1);
	c.registerDS(&f8->data_2);
	d.registerDS(&f8->data_3);
	e.registerDS(&f8->data_4);
	f.registerDS(&f8->data_5);
	g.registerDS(&f8->data_6);
	h.registerDS(&f8->data_7);

	expression->ops.push_back(move(f8));
	return *this;
}

QueryBuilder::ExpressionBuilder&
QueryBuilder::ExpressionBuilder::addOp(primitives::F9 op, DS a, DS b, DS c, DS d, DS e, DS f, DS g, DS h, DS i) {
	auto f9 = make_unique<F9_Op>(a, b, c, d, e, f, g, h, i, op);
	a.registerDS(&f9->data_0);
	b.registerDS(&f9->data_1);
	c.registerDS(&f9->data_2);
	d.registerDS(&f9->data_3);
	e.registerDS(&f9->data_4);
	f.registerDS(&f9->data_5);
	g.registerDS(&f9->data_6);
	h.registerDS(&f9->data_7);
	i.registerDS(&f9->data_8);

	expression->ops.push_back(move(f9));
	return *this;
}

QueryBuilder::ExpressionBuilder::operator std::unique_ptr<vectorwise::Expression>() { return move(expression); }

QueryBuilder::ExpressionBuilder::operator std::unique_ptr<vectorwise::Aggregates>() {
	auto r = make_unique<vectorwise::Aggregates>();
	r->ops = move(expression->ops);
	return r;
}

QueryBuilder::HashJoinBuilder::HashJoinBuilder(QueryBuilder& b)
    : base(b) {}
QueryBuilder::HashJoinBuilder::~HashJoinBuilder() { join->ht_entry_size += padding(join->ht_entry_size, 8); }

QueryBuilder::HashJoinBuilder QueryBuilder::HashJoin(DS probeMatches, pos_t (Hashjoin::*joinFun)()) {
	HashJoinBuilder b(*this);
	auto            nr   = nextOpNr();
	auto&           s    = operatorState.get<Hashjoin::Shared>(nr);
	auto            join = make_unique<Hashjoin>(s);
	join->followupIds    = reinterpret_cast<decltype(join->followupIds)>(vecs.getPlus1(sizeof(*join->followupIds)));
	join->followupEntries =
	    reinterpret_cast<decltype(join->followupEntries)>(vecs.getPlus1(sizeof(*join->followupEntries)));
	...
	b.join->buildMatches =
	    static_cast<runtime::Hashmap::EntryHeader**>(vecs.get(sizeof(runtime::Hashmap::EntryHeader*)));
	...
	auto scatter_hash = make_unique<FScatterOp>(primitives::scatter_hash_t_col,
	                                            b.buildHashBuffer,
	                                            reinterpret_cast<void**>(&b.join->scatterStart),
	                                            &b.join->ht_entry_size,
...
QueryBuilder::HashJoinBuilder::addBuildKey(DS col, primitives::F2 hash, primitives::FScatter scatter) {

	auto entryOffset = join->ht_entry_size;
	keyOffsets.push_back(entryOffset);
	join->ht_entry_size += col.data_size;
	...
	auto scatter_build = make_unique<FScatterOp>(
	    scatter, col, reinterpret_cast<void**>(&join->scatterStart), &join->ht_entry_size, entryOffset);
...
QueryBuilder::HashJoinBuilder&
QueryBuilder::HashJoinBuilder::addBuildKey(DS col, DS sel, primitives::F3 hash, primitives::FScatterSel scatter) {
	...
	auto scatter_build = make_unique<FScatterSelOp>(
	    scatter, sel, col, reinterpret_cast<void**>(&join->scatterStart), &join->ht_entry_size, entryOffset);
...
QueryBuilder::HashJoinBuilder&
QueryBuilder::HashJoinBuilder::addProbeKey(DS col, primitives::F2 hash, primitives::EQCheck eq) {
	...
	join->probeHashes = reinterpret_cast<runtime::Hashmap::hash_t*>(probeHashBuffer);

	// key equality
	auto keyEq = make_unique<EqualityCheck>(eq, (void**)join->buildMatches, entryOffset, join->probeMatches, col);
...
QueryBuilder::HashJoinBuilder&
QueryBuilder::HashJoinBuilder::addProbeKey(DS col, DS sel, primitives::F3 hash, primitives::EQCheck eq) {
	if (!probeHasSelection)
...
QueryBuilder::HashJoinBuilder&
QueryBuilder::HashJoinBuilder::addProbeKey(DS col, DS sel, primitives::F3 hash, DS selEq, primitives::EQCheck eq) {
	auto entryOffset = keyOffsets.front();
	keyOffsets.pop_front();
	...
	// key equality
	auto keyEq = make_unique<EqualityCheck>(eq, (void**)join->buildMatches, entryOffset, selEq, col);
...
QueryBuilder::HashJoinBuilder& QueryBuilder::HashJoinBuilder::addBuildValue(DS                   source,
                                                                            primitives::FScatter scatter,
                                                                            DS                   target,
                                                                            primitives::FGather  gather) {
	auto entryOffset = join->ht_entry_size;
	join->ht_entry_size += source.data_size;

	// build scatter
	auto scatter_build = make_unique<FScatterOp>(
	    scatter, source, reinterpret_cast<void**>(&join->scatterStart), &join->ht_entry_size, entryOffset);
	source.registerDS(&scatter_build->get<0>());
	join->buildScatter += move(scatter_build);
	// gather
	auto gather_build = make_unique<GatherOpCol>(gather, (void**)join->buildMatches, entryOffset, target);
	join->buildGather.ops.push_back(move(gather_build));
	return *this;
}

QueryBuilder::HashJoinBuilder& QueryBuilder::HashJoinBuilder::addBuildValue(
    DS source, DS sel, primitives::FScatterSel scatter, DS target, primitives::FGather gather) {
	...
	// build scatter
	auto scatter_build = make_unique<FScatterSelOp>(
	    scatter, sel, source, reinterpret_cast<void**>(&join->scatterStart), &join->ht_entry_size, entryOffset);
...
QueryBuilder::HashJoinBuilder& QueryBuilder::HashJoinBuilder::setProbeSelVector(DS sel, pos_t (Hashjoin::*joinFun)()) {
...
QueryBuilder::HashJoinBuilder& QueryBuilder::HashJoinBuilder::pushProbeSelVector(DS sel, DS target) {
	if (probeHasSelection)
	...
	auto lookup = move(base.Expression().addOp(primitives::lookup_sel, target, base.Value(join->probeMatches), sel));
	join->keyEquality.ops.push_back(move(lookup.expression->ops.back()));
	return *this;
}

QueryBuilder::HashGroupBuilder::HashGroupBuilder(QueryBuilder& b)
    : base(b) {}
...
QueryBuilder::HashGroupBuilder QueryBuilder::HashGroup() {
	...
	local.groupHashes   = static_cast<runtime::Hashmap::hash_t*>(vecs.get(8 /*sizeof(runtime::Hashmap::hash_t)*/));
	local.htMatches   = static_cast<runtime::Hashmap::EntryHeader**>(vecs.get(sizeof(runtime::Hashmap::EntryHeader*)));
	local.groupsFound = static_cast<pos_t*>(vecs.get(sizeof(pos_t)));
	local.groupsNotFound   = reinterpret_cast<SizeBuffer<pos_t>*>(vecs.getSizeBuffer(sizeof(pos_t)));
	local.keysNEq          = reinterpret_cast<SizeBuffer<pos_t>*>(vecs.getSizeBuffer(sizeof(pos_t)));
	...
	// scatter hash
	auto scatter_build = make_unique<FScatterSelOp>(primitives::scatter_sel_hash_t_col,
	                                                Value(local.groupRepresentatives),
	                                                Value(local.groupHashes),
	                                                reinterpret_cast<void**>(&local.scatterStart),
	                                                &local.ht_entry_size,
	                                                offsetof(runtime::Hashmap::EntryHeader, hash));
	local.buildScatter += move(scatter_build);

	auto scatter_build_global = make_unique<FScatterSelRowOp>(primitives::scatter_sel_row_hash_t_col,
	                                                          Value(global.groupRepresentatives),
	                                                          &global.rowData,
	                                                          &global.rowSize,
	                                                          0,
	                                                          reinterpret_cast<void**>(&global.scatterStart),
	                                                          &global.ht_entry_size,
	                                                          offsetof(runtime::Hashmap::EntryHeader, hash));
...
QueryBuilder::DS QueryBuilder::X(QueryBuilder::ScanBuilder& scan_bdr, std::string attribute, int sub_c) {
	DS r;

	if (sub_c == 0) {
		r.buf = DataStorage::BufferSpec::MTD;
	} else {
		r.buf = DataStorage::BufferSpec::X;
	}
	auto& attr  = scan_bdr.rel[attribute];
	r.data_size = attr.type->rt_size();
	r.data      = attr.data(sub_c);
	r.scan      = &scan_bdr.scan;
	return r;
}
...
QueryBuilder::HashGroupBuilder::~HashGroupBuilder() {
	...
	auto rowSize = local.ht_entry_size - sizeof(runtime::Hashmap::EntryHeader::next);
	// TODO: this seems wrong!
	auto& spill     = op.shared.spillStorage.create(runtime::this_worker->group->size * 4, rowSize);
	op.nrPartitions = spill.getPartitions().size();
	global.rowSize  = rowSize;
}

QueryBuilder::HashGroupBuilder& QueryBuilder::HashGroupBuilder::addKey(DS             col,
                                                                       primitives::F2 hash,
                                                                       /**** global aggr *****/
                                                                       ...
                                                                       /**** output *****/
                                                                       primitives::FGatherVal gather,
                                                                       DS                     out) {
	auto& op          = *this->group;
	auto& local       = op.preAggregation;
	auto& global      = op.globalAggregation;
	auto  entryOffset = local.ht_entry_size;
	local.ht_entry_size += col.data_size;
	global.ht_entry_size += col.data_size;

	auto partitionOffset = entryOffset - sizeof(runtime::Hashmap::EntryHeader::next);

	op.groupHash += base.Expression().addOp(hash, base.Value(local.groupHashes), col);

	auto neqCheck = make_unique<NEqualityCheck>(
	    eq, local.groupsFound, reinterpret_cast<void**>(local.htMatches), col, entryOffset, local.keysNEq);
	col.registerDS(&neqCheck->probeKey);
	local.keyEquality += move(neqCheck);

	auto neqCheckGlobal = make_unique<NEQCheckRowOp>(eqG,
	                                                 global.groupsFound,
	                                                 reinterpret_cast<void**>(global.htMatches),
	                                                 &global.rowData,
	                                                 entryOffset,
	                                                 &global.rowSize,
	                                                 partitionOffset,
	...
	auto partition = make_unique<FPartitionByKeyOp>(partitionByKey,
	                                                localLookup.unpartitionedRows,
	                                                col,
	                                                local.groupHashes,
	                                                localLookup.partitionEndsIn,
	                                                localLookup.partitionedRows,
	                                                localLookup.partitionEndsOut,
	                                                op.groupHt.get());
	...
	auto partitionGlobal = make_unique<FPartitionByKeyRowOp>(partitionByKeyG,
	                                                         globalLookup.unpartitionedRows,
	                                                         &global.rowData,
	                                                         0,
	                                                         &global.rowSize,
	                                                         partitionOffset,
	                                                         globalLookup.partitionEndsIn,
	                                                         globalLookup.partitionedRows,
	                                                         globalLookup.partitionEndsOut,
	                                                         op.groupHt.get());
	...
	auto scatter_build = make_unique<FScatterSelOp>(scatter,
	                                                base.Value(local.groupRepresentatives),
	                                                col,
	                                                reinterpret_cast<void**>(&local.scatterStart),
	                                                &local.ht_entry_size,
	                                                entryOffset);
	...
	auto scatter_build_global = make_unique<FScatterSelRowOp>(scatterG,
	                                                          base.Value(global.groupRepresentatives),
	                                                          &global.rowData,
	                                                          &global.rowSize,
	                                                          partitionOffset,
	                                                          reinterpret_cast<void**>(&global.scatterStart),
	...
	auto gather_groups = make_unique<GatherOpVal>(
	    gather, reinterpret_cast<void**>(global.htMatches), entryOffset, &global.ht_entry_size, out);
...
QueryBuilder::HashGroupBuilder& QueryBuilder::HashGroupBuilder::addKey(
    ...
    primitives::FPartitionByKeySel partitionByKey,
    DS                             selScat,
    primitives::FScatterSel        scatter,
    /**** global aggr *****/
    primitives::NEQCheckRow        eqG,
	...
	op.groupHash += base.Expression().addOp(hash, sel, base.Value(local.groupHashes), col);

	auto neqCheck = make_unique<NEqualityCheckSel>(
	    eq, local.groupsFound, reinterpret_cast<void**>(local.htMatches), sel, col, entryOffset, local.keysNEq);
	...
	auto partition = make_unique<FPartitionByKeySelOp>(partitionByKey,
	                                                   localLookup.unpartitionedRows,
	                                                   sel,
	                                                   col,
	                                                   local.groupHashes,
	...
	auto scatter_build = make_unique<FScatterSelOp>(
	    scatter, selScat, col, reinterpret_cast<void**>(&local.scatterStart), &local.ht_entry_size, entryOffset);
	col.registerDS(&scatter_build->get<1>());
	local.buildScatter += move(scatter_build);
...
QueryBuilder::HashGroupBuilder& QueryBuilder::HashGroupBuilder::addValue(DS                     col,
                                                                         primitives::FAggrInit  aggrInit,
                                                                         primitives::FAggr      aggr,
                                                                         primitives::FAggrRow   aggrGlobal,
                                                                         primitives::FGatherVal gather,
                                                                         DS                     out) {
	...
	auto aggregateInitOp = make_unique<FAggrInitOp>(
	    aggrInit, reinterpret_cast<void**>(&local.scatterStart), &local.ht_entry_size, entryOffset);
	local.buildScatter += move(aggregateInitOp);

	auto aggregateInitGlobalOp = make_unique<FAggrInitOp>(
	    aggrInit, reinterpret_cast<void**>(&global.scatterStart), &global.ht_entry_size, entryOffset);
	global.buildScatter += move(aggregateInitGlobalOp);

	auto aggr_op = make_unique<FAggrOp>(aggr, reinterpret_cast<void**>(local.htMatches), col, entryOffset);
	col.registerDS(&aggr_op->get<1>());
	op.updateGroups += move(aggr_op);

	auto aggrGlobal_op = make_unique<FAggrRowOp>(aggrGlobal,
	                                             reinterpret_cast<void**>(global.htMatches),
	                                             entryOffset,
	                                             &global.rowData,
	                                             &global.rowSize,
...
QueryBuilder::HashGroupBuilder& QueryBuilder::HashGroupBuilder::addValue(DS                     col,
	...
	auto aggr_op = make_unique<FAggrSelOp>(aggr, reinterpret_cast<void**>(local.htMatches), sel, col, entryOffset);
...
QueryBuilder::HashGroupBuilder& QueryBuilder::HashGroupBuilder::pushKeySelVec(DS sel, DS outBuf) {
	// add lookup to keys_equal
	auto lookup = move(base.Expression().addOp(
	    primitives::lookup_sel, outBuf, base.Value(group->preAggregation.groupRepresentatives), sel));
	group->preAggregation.buildScatter += move(lookup);
	return *this;
}

QueryBuilder::HashGroupBuilder& QueryBuilder::HashGroupBuilder::padToAlign(size_t align) {
	group->preAggregation.ht_entry_size += padding(group->preAggregation.ht_entry_size, align);
	group->globalAggregation.ht_entry_size += padding(group->globalAggregation.ht_entry_size, align);
	return *this;
}
} // namespace vectorwise
