...
//				if (event.fd < 0) std::cerr << "Error opening perf event " << ev.first << std::endl;
	...
	static double alp_profile(std::function<void()> fn, uint64_t repetitions = 1);
};
...
void PerfEvents::timeAndProfile(
	...
	std::cout << setw(20) << name << "," << setw(printFieldWidth) << (runtime * 1e3 / repetitions) << ",";
#ifdef __linux__
...
namespace profile {
double profile(std::function<void()> fn, uint64_t rep_c, uint64_t warmup_rep_c) {
	using namespace std;
	// warmup round
