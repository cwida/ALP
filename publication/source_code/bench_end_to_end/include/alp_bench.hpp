#ifndef ALP_COMPRESSION_ALP_BENCH_ALP_BENCH_HPP
#define ALP_COMPRESSION_ALP_BENCH_ALP_BENCH_HPP
...
#define BENCHMARK_UNUSED ALP_BENCH_MAYBE_UNUSED
...
// clang-format off
    ...
    #define ALP_BENCH_MAYBE_UNUSED __attribute__((unused))
#else
 ...
 * Now api has been added to be compatible with the rest of alp_bench.
 */
#if defined(__linuxp__)
namespace perf {
...
ALP_BENCH_MAYBE_UNUSED static uint64_t get_counters() {
#if defined(__aarch64__)
#if defined(__APPLE__)
...
ALP_BENCH_MAYBE_UNUSED static void configure_rdtsc() {
#if defined(__aarch64__)
#if defined(__APPLE__)
...
ALP_BENCH_MAYBE_UNUSED static void SleepForSeconds(double seconds) {
...
ALP_BENCH_MAYBE_UNUSED static void AppendHumanReadable(int n, std::string* str) {
	std::stringstream ss;
...
ALP_BENCH_MAYBE_UNUSED static std::string HumanReadableNumber(double n, double one_k = 1024.0) {
...
ALP_BENCH_MAYBE_UNUSED static bool GetSysctl(std::string const& name, std::string* out) {
	out->clear();
...
ALP_BENCH_MAYBE_UNUSED std::string GetSystemName() {
...
ALP_BENCH_MAYBE_UNUSED static std::string CsvEscape(const std::string& s) {
	std::string tmp;
...
ALP_BENCH_MAYBE_UNUSED static BenchmarkBuilder create(const std::string& name) {
	cycleclock::Init();
