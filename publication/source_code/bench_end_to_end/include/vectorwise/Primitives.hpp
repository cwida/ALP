...
namespace vectorwise { namespace primitives {
...
template <typename T, template <typename> class Op>
pos_t sel_col_val_or_val(pos_t n, pos_t* RES result, T* RES param1, T* RES param2, T* RES param3)
...
template <typename T, template <typename> class Op>
pos_t selsel_col_val(pos_t n, pos_t* RES inSel, pos_t* RES result, T* RES param1, T* RES param2)
...
template <typename T, template <typename> class Op>
pos_t selsel_col_col(pos_t n, pos_t* RES inSel, pos_t* RES result, T* RES param1, T* RES param2)
/// select with input selection vector and two columns
{
	uint64_t found = 0;
	for (uint64_t i = 0; i < n; ++i) {
...
template <typename T, template <typename> class Op>
pos_t selsel_col_val_bf(pos_t n, pos_t* RES inSel, pos_t* RES result, T* RES param1, T* RES param2)
/// select with input selection vector and two columns
{
	auto       rStart = result;
	const auto con    = *param2;
...
template <typename T, template <typename> class Op>
pos_t selsel_col_col_bf(pos_t n, pos_t* RES inSel, pos_t* RES result, T* RES param1, T* RES param2)
/// select with input selection vector and two columns
{
	auto rStart = result;
	for (uint64_t i = 0; i < n; ++i) {
...
template <typename T, template <typename> class Op>
pos_t proj_sel_col_val(pos_t n, pos_t* RES inSel, T* RES result, T* RES param1, T* RES param2)
...
template <typename T, template <typename> class Op>
pos_t proj_sel_val_col(pos_t n, pos_t* RES inSel, T* RES result, T* RES param1, T* RES param2)
...
template <typename T, template <typename> class Op>
pos_t proj_sel_both_col_col(pos_t n, pos_t* RES inSel, T* RES result, T* RES param1, T* RES param2)
...
template <typename T, template <typename> class Op>
pos_t proj_sel_col_col(pos_t n, pos_t* RES inSel, T* RES result, T* RES param1, T* RES param2)
...
template <typename T, template <typename> class Op>
pos_t proj_col_sel_col(pos_t n, pos_t* RES inSel, T* RES result, T* RES param1, T* RES param2)
...
template <typename T, template <typename> class Op>
pos_t proj_sel_col_sel_col(pos_t n, pos_t* RES inSel1, pos_t* RES inSel2, T* RES result, T* RES param1, T* RES param2)
...
template <typename T, template <typename> class Op>
pos_t aggr_static_sel_col(pos_t n, pos_t* RES inSel, T* RES result, T* RES param1)
...
template <typename T, template <typename> class Op>
pos_t aggr_sel_col(pos_t n, T* RES entries[], pos_t* selParam1, T* RES param1, size_t offset)
...
template <typename T, template <typename> class Op>
pos_t aggr_row(pos_t n, T* RES entries[], size_t offset, T** RES dataPtr, size_t* inSizePtr, size_t inOffset)
/// aggregate into multiple aggregators given by result
{
	auto data   = addBytes(*dataPtr, inOffset);
	auto inSize = *inSizePtr;
	for (auto e = entries, end = e + n; e < end; ++e, data = addBytes(data, inSize)) {
		auto aggregate = addBytes(*e, offset);
...
struct OpTraits;
template <typename T>
struct OpTraits<T, std::plus> {
	static const T neutral = 0;
};
template <typename T>
struct OpTraits<T, std::minus> {
	static const T neutral = 0;
};
template <typename T>
struct OpTraits<T, std::multiplies> {
	static const T neutral = 1;
};
template <typename T>
struct OpTraits<T, std::divides> {
	static const T neutral = 1;
};
template <typename T>
struct OpTraits<T, std::modulus> {
	static const T neutral = 1;
};
...
//--- scatter templates
template <typename T>
pos_t scatter(pos_t n, T* RES input, T** RES start, size_t* step, size_t offset) {
	const auto s       = *step;
...
template <typename T>
pos_t scatter_sel(pos_t n, pos_t* RES inSel, T* RES input, T** RES start, size_t* step, size_t offset) {
	const auto s       = *step;
	auto       current = addBytes(*start, offset);
...
/// gathers using a list of pointers
{
	for (size_t i = 0; i < n; ++i)
		out[i] = *addBytes(input[i], offset);
	return n;
}

template <typename T>
pos_t gather_sel_col(pos_t n, pos_t* RES inSel, T* RES input[], size_t offset, T* RES out)
...
template <typename T>
pos_t gather_val(pos_t n, T** RES input, size_t offset, size_t* struct_size, T* RES out)
...
template <typename T, template <typename> class Op>
pos_t keys_equal(pos_t n, T* RES buildEntry[], size_t offset, pos_t* probeIdx, T* RES probeKey)
...
template <typename T>
pos_t keys_not_equal(
    pos_t n, pos_t* RES entryIdx, T* RES entry[], T* RES probeKey, size_t offset, SizeBuffer<pos_t>* RES notEq)
/// takes join result Entry*[], probeIdx, compares keys for equality and writes
...
template <typename T>
pos_t keys_not_equal_sel(pos_t                  n,
                         pos_t* RES             entryIdx,
                         T* RES                 entry[],
                         pos_t* RES             probeSel,
                         T* RES                 probeKey,
...
template <typename T>
pos_t keys_not_equal_sel_bf(pos_t                  n,
                            pos_t* RES             entryIdx,
                            T* RES                 entry[],
                            pos_t* RES             probeSel,
                            T* RES                 probeKey,
...
template <typename T>
pos_t keys_not_equal_row(pos_t                  n,
                         pos_t* RES             entryIdx,
                         T* RES                 entry[],
                         T** RES                probeKeyPtr,
                         size_t                 offset,
                         size_t*                entrySizePtr,
                         size_t                 rowEntryOffset,
                         SizeBuffer<pos_t>* RES notEq)
/// takes join result Entry*[], probeIdx, compares keys for equality and writes
/// back key ids that did not match
...
//--- partitioning by key
template <typename T>
pos_t partition_by_key(pos_t                                n,
                       pos_t*                               sel,
                       T*                                   keys,
                       hash_t*                              hashes,
                       pos_t*                               partitionBounds,
                       pos_t*                               selOut,
...
///        ht: a hashtable used during partitioning
{
	...
	for (pos_t p = 0; p < n; ++p) { // each partition
		...
		// Pass 1: Determine frequency for each value
		{
			...
			auto& key  = keys[s];
			auto& hash = hashes[s];
			auto  el   = ht->findFirst(hash, [&](pos_t pos) { return key == keys[pos]; });
			if (el)
				(*el)++;
			else {
				pos_t one = 1;
		...
		// Pass 2: Use offsets to scatter sel indices
		{
			auto  pos             = sel[i];
			auto& key             = keys[pos];
			auto& groupOffset     = *(ht->findFirst(hashes[pos], [&](pos_t cand) { return key == keys[cand]; }));
...
template <typename T>
pos_t partition_by_key_sel(pos_t                                n,
                           pos_t*                               sel,
                           pos_t*                               selK,
                           T*                                   keys,
                           hash_t*                              hashes,
...
///        ht: a hashtable used during partitioning
{
	...
	for (pos_t p = 0; p < n; ++p) { // each partition
		...
		// Pass 2: Use frequency to scatter sel indices
		{
			...
			auto  pos             = selK[idx];
			auto& key             = keys[pos];
			auto& groupOffset     = *(ht->findFirst(hashes[idx], [&](pos_t cand) { return key == keys[cand]; }));
			selOut[groupOffset++] = sel[i];
		}
		ht->clear([&](auto& ht) {
			for (pos_t i = partitionStart, end = partitionBounds[p]; i < end; ++i) {
...
template <typename T>
pos_t partition_by_key_row(pos_t                                n,
                           pos_t*                               sel,
                           T**                                  rowDataStartPtr,
                           size_t                               hashOffset,
                           size_t*                              entrySizePtr,
...
///        ht: a hashtable used during partitioning
{
	...
	for (pos_t p = 0; p < n; ++p) { // each partition
		...
		// Pass 1: Determine frequency for each value
		{
			...
			auto& hash = *addBytes((hash_t*)rowDataStart, pos * entrySize + hashOffset);
			auto  el   = ht->findFirst(
                hash, [&](pos_t cand) { return key == *addBytes(rowDataStart, cand * entrySize + dataOffset); });
			if (el)
				(*el)++;
			else {
				pos_t one = 1;
...
using F4 = pos_t (*)(pos_t n, void* inputSel, void* result, void* param1, void* param2);
using F5 = pos_t (*)(pos_t n, void* param1, void* param2, void* param3, void* param4, void* param5);
using F6 = pos_t (*)(pos_t n, void* param1, void* param2, void* param3, void* param4, void* param5, void* param6);
using F7 = pos_t (*)(
    pos_t n, void* param1, void* param2, void* param3, void* param4, void* param5, void* param6, void* param7);
using F8 = pos_t (*)(pos_t n,
                     void* param1,
                     void* param2,
                     void* param3,
                     void* param4,
...
using EQCheck = size_t (*)(pos_t n, void* buildEntry[], size_t offset, pos_t* probeIdx, void* probeKey);
/// primitive to perform key comparison and write out non matching keys
using NEQCheck = pos_t (*)(
    pos_t n, pos_t* RES entryIdx, void* RES entry[], void* RES probeKey, size_t offset, SizeBuffer<pos_t>* notEq);
using NEQCheckSel = pos_t (*)(pos_t              n,
                              pos_t* RES         entryIdx,
                              void* RES          entry[],
                              pos_t*             probeSel,
                              void* RES          probeKey,
...
using NEQCheckRow = pos_t (*)(pos_t                  n,
                              pos_t* RES             entryIdx,
                              void* RES              entry[],
                              void** RES             probeKeyPtr,
                              size_t                 offset,
                              size_t*                entrySizePtr,
                              size_t                 rowEntryOffset,
                              SizeBuffer<pos_t>* RES notEq);
using FScatter    = pos_t (*)(pos_t n, void* RES input, void** RES start, size_t* step, size_t offset);
using FScatterSel =
    pos_t (*)(pos_t n, pos_t* RES inSel, void* RES input, void** RES start, size_t* step, size_t offset);
using FScatterSelRow = pos_t (*)(pos_t      n,
                                 pos_t* RES inSel,
                                 void** RES inputPtr,
                                 size_t*    inStepPtr,
                                 size_t     inOffset,
...
using FGather    = pos_t (*)(pos_t n, void* RES input[], size_t offset, void* RES out);
using FGatherSel = pos_t (*)(pos_t n, pos_t* RES inSel, void* RES input[], size_t offset, void* RES out);
using FGatherVal = pos_t (*)(pos_t n, void** RES input, size_t offset, size_t* struct_size, void* RES out);
/// function type for aggregation
using FAggr    = pos_t (*)(pos_t n, void* RES result[], void* RES param1, size_t offset);
using FAggrSel = pos_t (*)(pos_t n, void* RES result[], pos_t* selParam1, void* RES param1, size_t offset);
using FAggrRow =
    pos_t (*)(pos_t n, void* RES entries[], size_t offset, void** RES dataPtr, size_t* inSizePtr, size_t inOffset);
using FAggrInit = pos_t (*)(pos_t n, void** RES toInit, size_t* struct_size, size_t offset);
/// function types for partitioning
using FPartitionByKey    = pos_t (*)(pos_t   n,
                                  pos_t*  sel,
                                  void*   keys,
                                  hash_t* hashes,
                                  pos_t*  partitionBounds,
...
using FPartitionByKeySel = pos_t (*)(pos_t                                n,
                                     pos_t*                               sel,
                                     pos_t*                               selK,
                                     void*                                keys,
                                     hash_t*                              hashes,
                                     ...
                                     pos_t*                               partitionBoundsOut,
                                     runtime::HashmapSmall<pos_t, pos_t>* ht);
using FPartitionByKeyRow = pos_t (*)(pos_t  n,
                                     pos_t* sel,
...
#define EACH_COMP(m, c)           m(c, equal_to) m(c, greater_equal) m(c, greater) m(c, less_equal) m(c, less)
...
#define EACH_TYPE_BASIC(m, c)                                                                                          \
	m(Date, c) m(Char_1, c) m(Char_6, c) m(Char_7, c) m(Char_9, c) m(Char_10, c) m(Char_12, c) m(Char_15, c)           \
	    m(Char_25, c) m(hash_t, c)
#define EACH_TYPE_FULL(m, c)                   m(int32_t, c) m(int64_t, c) m(int8_t, c) m(int16_t, c)
#define EACH_TYPE(m, c)                        EACH_TYPE_BASIC(m, c) EACH_TYPE_FULL(m, c)

#define NIL(t, m)                              m(t)

#define MK_SEL_COLCOL_DECL(type, op)           extern F3 sel_##op##_##type##_col_##type##_col;
#define MK_SEL_COLVAL_DECL(type, op)           extern F3 sel_##op##_##type##_col_##type##_val;
#define MK_SEL_COLVALORVAL_DECL(type, op)      extern F4 sel_##op##_##type##_col_##type##_val_or_##type##_val;
#define MK_SELSEL_COLCOL_DECL(type, op)        extern F4 selsel_##op##_##type##_col_##type##_col;
#define MK_SELSEL_COLVAL_DECL(type, op)        extern F4 selsel_##op##_##type##_col_##type##_val;

#define MK_SEL_COLCOL_BF_DECL(type, op)        extern F3 sel_##op##_##type##_col_##type##_col_bf;
#define MK_SEL_COLVAL_BF_DECL(type, op)        extern F3 sel_##op##_##type##_col_##type##_val_bf;
#define MK_SELSEL_COLCOL_BF_DECL(type, op)     extern F4 selsel_##op##_##type##_col_##type##_col_bf;
#define MK_SELSEL_COLVAL_BF_DECL(type, op)     extern F4 selsel_##op##_##type##_col_##type##_val_bf;

#define MK_PROJ_COLCOL_DECL(type, op)          extern F3 proj_##op##_##type##_col_##type##_col;
#define MK_PROJ_COLVAL_DECL(type, op)          extern F3 proj_##op##_##type##_col_##type##_val;
#define MK_PROJ_VALCOL_DECL(type, op)          extern F3 proj_##op##_##type##_val_##type##_col;
#define MK_PROJ_SEL_BOTH_COLCOL_DECL(type, op) extern F4 proj_sel_both_##op##_##type##_col_##type##_col;
#define MK_PROJ_SEL_COLCOL_DECL(type, op)      extern F4 proj_##op##_sel_##type##_col_##type##_col;
#define MK_PROJ_COL_SEL_COL_DECL(type, op)     extern F4 proj_##op##_##type##_col_sel_##type##_col;
#define MK_PROJ_SEL_COL_SEL_COL_DECL(type, op) extern F5 proj_##op##_sel_##type##_col_sel_##type##_col;
#define MK_PROJ_SEL_COLVAL_DECL(type, op)      extern F4 proj_sel_##op##_##type##_col_##type##_val;
#define MK_PROJ_SEL_VALCOL_DECL(type, op)      extern F4 proj_sel_##op##_##type##_val_##type##_col;

#define MK_AGGR_STATIC_COL_DECL(type, op)      extern F2 aggr_static_##op##_##type##_col;
#define MK_AGGR_STATIC_SEL_COL_DECL(type, op)  extern F3 aggr_static_sel_##op##_##type##_col;
#define MK_AGGR_COL_DECL(type, op)             extern FAggr aggr_##op##_##type##_col;
#define MK_AGGR_SEL_COL_DECL(type, op)         extern FAggrSel aggr_sel_##op##_##type##_col;
#define MK_AGGR_ROW_DECL(type, op)             extern FAggrRow aggr_row_##op##_##type##_col;
#define MK_AGGR_INIT_DECL(type, op)            extern FAggrInit aggr_init_##op##_##type##_col;
...
#define MK_SCATTER_SEL_ROW_DECL(type)          extern FScatterSelRow scatter_sel_row_##type##_col;

#define MK_GATHER_COL_DECL(type)               extern FGather gather_col_##type##_col;
#define MK_GATHER_SEL_COL_DECL(type)           extern FGatherSel gather_sel_col_##type##_col;
...
#define MK_KEYS_NOT_EQUAL_DECL(type)           extern NEQCheck keys_not_equal_##type##_col;
#define MK_KEYS_NOT_EQUAL_SEL_DECL(type)       extern NEQCheckSel keys_not_equal_sel_##type##_col;
#define MK_KEYS_NOT_EQUAL_ROW_DECL(type)       extern NEQCheckRow keys_not_equal_row_##type##_col;

#define MK_PARTITION_DECL(type)                extern FPartitionByKey partition_by_key_##type##_col;
#define MK_PARTITION_SEL_DECL(type)            extern FPartitionByKeySel partition_by_key_sel_##type##_col;
#define MK_PARTITION_ROW_DECL(type)            extern FPartitionByKeyRow partition_by_key_row_##type##_col;
...
#endif
}} // namespace vectorwise::primitives
