...
static void prepare_buffers() {
    for (long i = 0; i < num_preallocated_buffers; i++) {
        ...
        streambufs.emplace_back(Aws::New<Aws::Utils::Stream::PreallocatedStreamBuf>(allocation_tag, streambufarrays[i].data(), static_cast<uint64_t>(part_size)));
...
static void GetObjects(const s3_client_t& s3_client, const Aws::String& fromBucket, long object_count, long object_size)
{
    for (long current_object = 0; current_object < object_count; current_object++) {
        ...
        // Make request in chunks of part_size. Otherwise, we would try to store the whole file in memory.
...
static void usage(const char *program) {
    std::cerr << "Usage: " << program << " region bucket object_count object_size part_size num_pre_allocate repetitions target_throughput" << std::endl;
    exit(EXIT_FAILURE);
}
...
int main(int argc, char **argv) {
    ...
    Aws::InitAPI(options);
    {
        ...
        std::size_t total_size_bytes = static_cast<size_t >(object_count) * static_cast<size_t >(object_size);
        double total_size_gigabits = static_cast<double>(total_size_bytes * 8) / static_cast<double>(1<<30);
        double gbps = (total_size_gigabits / s) * static_cast<double>(repetitions);
        std::cout << "total_size_bytes = " << total_size_bytes << " total_downloaded_size = " << total_downloaded_size << std::endl;
