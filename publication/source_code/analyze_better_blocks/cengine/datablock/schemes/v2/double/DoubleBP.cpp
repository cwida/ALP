...
namespace d {
// -------------------------------------------------------------------------------------
u32 DoubleBP::compress(const DOUBLE *src, const BITMAP *, u8 *dest, DoubleStats& stats, [[maybe_unused]] u8 allowed_cascading_level)
{
   ...
   // -------------------------------------------------------------------------------------
   size_t compressed_codes_size = 4*stats.tuple_count + 1024; // give FBP a large enough output buffer
   // 2x tuple count because we're actually compressing doubles
   codec.compress(reinterpret_cast<const u32 *>(src), stats.tuple_count*2, dest_4_aligned, compressed_codes_size);
   col_struct.u32_count = compressed_codes_size;
   // -------------------------------------------------------------------------------------
   return sizeof(DoubleBPStructure) + compressed_codes_size * sizeof(u32);
}
// -------------------------------------------------------------------------------------
void DoubleBP::decompress(DOUBLE *dest, [[maybe_unused]] BitmapWrapper *, const u8 *src, u32 tuple_count, [[maybe_unused]] u32 level)
{
   ...
   const u32* dst_ptr = codec.decompress(encoded_array, col_struct.u32_count, reinterpret_cast<u32 *>(dest), decompressed_codes_size);
   if ( dst_ptr != encoded_array + col_struct.u32_count ) {
      throw Generic_Exception("Decompressing DoubleBP failed");
   }
}
// -------------------------------------------------------------------------------------
DOUBLE DoubleBP::lookup(u32) { UNREACHABLE(); };
// -------------------------------------------------------------------------------------
