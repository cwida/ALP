...
inline double FDictExpectedCompressionRatio(StatsType &stats)
{
   if ( stats.unique_count > (std::numeric_limits<CodeType>::max() + 1)) {
      ...
      const u32 after_size = sizeof(FixedDictionaryStructure<NumberType>) + (stats.unique_count * sizeof(NumberType)) + (stats.tuple_count * sizeof(CodeType));
...
inline void FDictDecompressColumn(NumberType *dest, BitmapWrapper *, const u8 *src, u32 tuple_count, u32 level)
{
   // -------------------------------------------------------------------------------------
