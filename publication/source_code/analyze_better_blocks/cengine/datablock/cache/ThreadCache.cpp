...
{
   get().dump_meta.rel_name = rel_name;
   get().dump_meta.col_name = col_name;
   get().dump_meta.col_type = col_type;
}
// -------------------------------------------------------------------------------------
void ThreadCache::dumpPush(string scheme_name, double cf, u32 before_size, u32 after_size, u32 unique_count, string comment)
{
   get().estimation_deviation_csv << get().dump_meta.rel_name << '\t'
                                  << get().dump_meta.col_name << '\t'
                                  << get().dump_meta.col_type << '\t'
                                  << get().dump_meta.chunk_i << '\t'
                                  << get().compression_level << '\t'
                                  << scheme_name << '\t'
                                  << cf << '\t'
                                  << before_size << '\t'
                                  << after_size << '\t'
                                  << CD(before_size) / CD(after_size) << '\t'
                                  << comment << '\t'
                                  << unique_count << '\n';
}
// -------------------------------------------------------------------------------------
...
{
   get().fsst_csv << get().dump_meta.rel_name << '\t'
                                  << get().dump_meta.col_name << '\t'
                                  << get().dump_meta.chunk_i << '\t'
                                  << before_total << '\t'
                                  << before_pool << '\t'
