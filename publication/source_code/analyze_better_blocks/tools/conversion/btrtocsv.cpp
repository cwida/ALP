...
void outputChunk(std::ofstream &csvstream, u32 tuple_count,
                 ...
                 std::vector<std::vector<cengine::db::BtrReader>> &readers,
    ...
    for (size_t row = 0; row < tuple_count; row++) {
        for (size_t col = 0; col < decompressed_columns.size(); col++) {
            cengine::db::BtrReader &reader = readers[col][counters[col].first];
                ...
                switch (reader.getColumnType()) {
                    ...
                    case ColumnType::STRING: {
                        ...
                        if (requires_copy[col]) {
                            auto string_pointer_array_viewer = cengine::StringPointerArrayViewer(reinterpret_cast<const u8 *>(decompressed_columns[col].data()));
                            data = string_pointer_array_viewer(row);
                        } else {
                            auto string_array_viewer = cengine::StringArrayViewer(reinterpret_cast<const u8 *>(decompressed_columns[col].data()));
...
int main(int argc, char **argv)
{
    ...
    // Open output file
    auto csvstream = ofstream(FLAGS_csv);
