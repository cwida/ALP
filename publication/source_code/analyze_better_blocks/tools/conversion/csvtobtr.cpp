//
// Created by david on 13.04.22.
//
// This program takes a csv file and its yaml schema and converts it to btr blocks
//
// Example call: csvtobtr TODO
//
...
// This one actually does nothing, but without it linking will fail.
DEFINE_string(fsst_stats, "", "");

void verify_or_die(const std::string& filename, const std::vector<cengine::InputChunk> &input_chunks) {
    if (!FLAGS_verify) {
        return;
    }
    ...
    for (SIZE chunk_i = 0; chunk_i < reader.getChunkCount(); chunk_i++) {
        ...
        if (!input_chunks[chunk_i].compareContents(output.data(), reader.getBitmap(chunk_i), reader.getTupleCount(chunk_i),
                                         requires_copy)) {
...
int main(int argc, char **argv)
{
    ...
    if (FLAGS_create_binary) {
        ...
        cengine::Parser::parse(FLAGS_csv, schema, FLAGS_binary);
    ...
    // Create relation
    cengine::Relation relation = cengine::Relation(schema, FLAGS_binary);
    std::filesystem::path yaml_path = FLAGS_yaml;
    relation.name = yaml_path.stem();

    // Prepare datastructures for btr compression
    //auto ranges = relation.getRanges(static_cast<cengine::SplitStrategy>(1), 9999);
    auto ranges = relation.getRanges(cengine::SplitStrategy::SEQUENTIAL, 9999);
...
//    }
    ...
    std::vector<atomic_size_t> sizes_uncompressed(relation.columns.size());
    std::vector<atomic_size_t> sizes_compressed(relation.columns.size());
    ...
    tbb::parallel_for(SIZE(0), relation.columns.size(), [&](SIZE column_i) {
        ...
        for (SIZE chunk_i = 0; chunk_i < ranges.size(); chunk_i++) {
            ...
            std::vector<u8> data = cengine::db::Datablock::compress(input_chunk);
            sizes_uncompressed[column_i] += input_chunk.size;

            if (!part.canAdd(data.size())) {
                std::string filename = path_prefix + to_string(part_counters[column_i]);
    ...
    cengine::db::Datablock::writeMetadata(FLAGS_btr + "/metadata", types, part_counters, ranges.size());
