...
static inline const cengine::db::FileMetadata *read_metatdata(const s3_client_t &s3_client) {
    ...
    auto metadata = reinterpret_cast<const cengine::db::FileMetadata *>(metadata_v.data());
...
static uint64_t decompressPart(long idx) {
    ...
    // Open Reader
    cengine::db::BtrReader reader(buffer);
...
static inline void requestColumn(const s3_client_t& s3_client, const cengine::db::FileMetadata *file_metadata, u32 column) {
...
int main(int argc, char **argv) {
    ...
    Aws::InitAPI(options);
    {
        ...
        // Read the metadata
        const cengine::db::FileMetadata *file_metadata = read_metatdata(s3_client);
