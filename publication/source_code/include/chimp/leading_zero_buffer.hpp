...
//                         DuckDB
//
// lwcbench/compression/chimp/leading_zero_buffer.hpp
//
//
//===----------------------------------------------------------------------===//
...
#ifdef DEBUG
#include "dependencies/assert.hpp"
#include "dependencies/vector.hpp"
#endif
...
public:
	...
	void Flush() {
		if ((counter & 7) != 0) { FlushBuffer(); }
	}

	uint64_t BitsWritten() const { return counter * 3; }
...
public:
	...
	inline uint64_t BlockIndex() const { return ((counter >> 3) * (LEADING_ZERO_BLOCK_BIT_SIZE / 8)); }
	...
	idx_t GetCount() const { return counter; }
	idx_t BlockCount() const { return (counter >> 3) + ((counter & 7) != 0); }
