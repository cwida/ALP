...
public:
	...
	DUCKDB_API constexpr hugeint_t(int64_t upper, uint64_t lower)
	    : lower(lower)
	    , upper(upper) {};
...
struct LogicalType {
	...
	inline const ExtraTypeInfo* AuxInfo() const { return type_info_.get(); }
	inline void                 CopyAuxInfo(const LogicalType& other) { type_info_ = other.type_info_; }
	...
	inline bool     operator!=(const LogicalType& rhs) const { return !(*this == rhs); }
	...
	DUCKDB_API static bool TypeIsTimestamp(const LogicalType& type) { return TypeIsTimestamp(type.id()); }
	DUCKDB_API string      ToString() const;
