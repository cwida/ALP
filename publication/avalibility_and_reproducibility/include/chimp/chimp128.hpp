...
//                         DuckDB
//
// lwcbench/compression/chimp/algorithm/chimp128.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	...
	static void CompressValue(CHIMP_TYPE in, State& state) {
		...
		if (((int64_t)state.ring_buffer.Size() - (int64_t)reference_index) < (int64_t)ChimpConstants::BUFFER_SIZE) {
			...
			if (current_index > state.ring_buffer.Size()) { current_index = 0; }
...
public:
	...
	static inline CHIMP_TYPE Load(ChimpConstants::Flags flag,
	                              uint8_t               leading_zeros[],
	                              uint32_t&             leading_zero_index,
	                              UnpackedData          unpacked_data[],
