               ...
               const Aws::String& fromBucket, const Aws::String& region)
{
    ...
    auto downloadHandle = transfer_manager->DownloadFile(fromBucket,
                                                         objectKey,
    ...
    for (int i = 0; i < 100; i++) {
        ...
        std::string result(read_buffer.GetUnderlyingData(), read_buffer.GetUnderlyingData() + stream->gcount());
