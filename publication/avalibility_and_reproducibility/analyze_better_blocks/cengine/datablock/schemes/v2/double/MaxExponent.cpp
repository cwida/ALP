 ...
 *
 * not necessarily slower than decimal, there is a chance to optimize, e.g don't start from e=0 but from the latest max_exponent
 *
 */
u32 MaxExponent::compress(const DOUBLE *src, const BITMAP *nullmap, u8 *dest, DoubleStats &stats, u8 allowed_cascading_level)
{
   ...
   for ( u32 row_i = 0; row_i < stats.tuple_count; row_i++ ) {
      ...
      for ( e = 0; e <= max_exponent; e++ ) {
         ...
         if ( if_converted_back == current_double && ((std::floor(std::log2(sd)) + 1) <= FLAGS_MaxExponent_siginifcant_digit_bits_limit)) {
            convertable = true;
            break;
         }
      }
   ...
   if ( sd_v.size()) {
      ...
      u32 used_space;
      IntegerSchemePicker::compress(sd_v.data(), nullptr, write_ptr, sd_v.size(), allowed_cascading_level - 1, used_space, col_struct.sd_scheme, AUTO_SCHEME, "significant digits");
      write_ptr += used_space;
      // -------------------------------------------------------------------------------------
      spdlog::debug("MaxExponent: sd_c = {} sd_s = {}", CI(col_struct.sd_scheme), CI(used_space));
      // -------------------------------------------------------------------------------------
   }
   // -------------------------------------------------------------------------------------
   // Compress patches
   {
      ...
      u32 used_space;
      DoubleSchemePicker::compress(p_v.data(), nullptr, write_ptr, p_v.size(), allowed_cascading_level - 1, used_space, col_struct.p_scheme, AUTO_SCHEME, "patches");
      write_ptr += used_space;
      // -------------------------------------------------------------------------------------
      spdlog::debug("MaxExponent: p_c = {} p_s = {}", CI(col_struct.p_scheme), CI(used_space));
      // -------------------------------------------------------------------------------------
   }
   // -------------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------------
void MaxExponent::decompress(DOUBLE *dest, BitmapWrapper *, const u8 *src, u32 tuple_count, u32 level)
{
