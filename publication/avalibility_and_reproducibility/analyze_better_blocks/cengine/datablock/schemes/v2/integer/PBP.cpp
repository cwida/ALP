...
u32 PBP::compress(const INTEGER *src, const BITMAP *, u8 *dest, SInteger32Stats &stats, u8)
{
   ...
   // -------------------------------------------------------------------------------------
   fast_pfor.compress(reinterpret_cast<const u32 *>(src), stats.tuple_count, dest_4_aligned, compressed_codes_size);
...
void PBP::decompress(INTEGER *dest, BitmapWrapper *, const u8 *src, u32 tuple_count, u32 level)
{
   ...
   auto encoded_array = const_cast<u32 *>(reinterpret_cast<const u32 *>(col_struct.data + col_struct.padding));
   if ( fast_pfor.decompress(encoded_array, col_struct.u32_count, reinterpret_cast<u32 *>(dest), decompressed_codes_size) != encoded_array + col_struct.u32_count ) {
      throw Generic_Exception("Decompressing XPBP failed");
   }
   assert(decompressed_codes_size == tuple_count);
}
// -------------------------------------------------------------------------------------
void PBP::scan(Predicate, BITMAP *, const u8 *, u32) { UNREACHABLE(); }
...
u32 PBP_DELTA::compress(const INTEGER *src, const BITMAP *, u8 *dest, SInteger32Stats &stats, u8)
{
   ...
   // TODO we may be able to get rid of this extra array and write directly to dest
   ...
   auto dest_4_aligned = reinterpret_cast<u32 *>(dest_integer);
   // -------------------------------------------------------------------------------------
   fast_pfor.compress(reinterpret_cast<const u32 *>(src_biased.get()), stats.tuple_count, dest_4_aligned, compressed_codes_size);
   col_struct.u32_count = compressed_codes_size;
   // -------------------------------------------------------------------------------------
   return sizeof(XPBPStructure) + compressed_codes_size * sizeof(u32);
}
// -------------------------------------------------------------------------------------
void PBP_DELTA::decompress(INTEGER *dest, BitmapWrapper *, const u8 *src, u32 tuple_count, u32 level)
{
   auto &col_struct = *reinterpret_cast<const XPBPStructure *> (src);
   // -------------------------------------------------------------------------------------
   FPFor codec;
   SIZE decompressed_codes_size = tuple_count;
   auto encoded_array = const_cast<u32 *>(reinterpret_cast<const u32 *>(col_struct.data + col_struct.padding));
   if ( codec.decompress(encoded_array, col_struct.u32_count, reinterpret_cast<u32 *>(dest), decompressed_codes_size) != encoded_array + col_struct.u32_count ) {
...
// -------------------------------------------------------------------------------------
void FBP::scan(Predicate, BITMAP *, const u8 *, u32) { UNREACHABLE(); }
...
// -------------------------------------------------------------------------------------
void EXP_FBP::scan(Predicate, BITMAP *, const u8 *, u32) { UNREACHABLE(); }
...
u32 FBP64::compress(u64 *src, u8 *dest, u32 tuple_count)
{
   ...
   auto dest_4_aligned = reinterpret_cast<u32 *>(dest_integer);
   // -------------------------------------------------------------------------------------
   fast_pfor.compress(reinterpret_cast<const u32 *>(src), tuple_count, dest_4_aligned, compressed_codes_size);
   col_struct.u32_count = compressed_codes_size;
   // -------------------------------------------------------------------------------------
   return sizeof(XPBPStructure) + compressed_codes_size * sizeof(u32);
}
