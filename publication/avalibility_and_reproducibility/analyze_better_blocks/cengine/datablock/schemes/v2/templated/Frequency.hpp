...
public:
   ...
   {
      // Threshold : stats.max larger than 1 byte (b/c PBP would do just as well as frequency)
      // top_1 occurrence pct >= 90%
...
// -------------------------------------------------------------------------------------
   static inline u32 compressColumn(const NumberType *src, const BITMAP *nullmap, u8 *dest, StatsType &stats, u8 allowed_cascading_level)
   {
      ...
      // -------------------------------------------------------------------------------------
      {
         // Compress exceptions
         u32 used_space;
         CSchemePicker<NumberType, SchemeType, StatsType, SchemeCodeType>::compress(exceptions.data(), nullptr, write_ptr, exceptions.size(), allowed_cascading_level - 1, used_space, col_struct.next_scheme, AUTO_SCHEME, "exceptions");
...
// -------------------------------------------------------------------------------------
static inline
void decompressColumn(NumberType *dest, BitmapWrapper *, const u8 *src, u32 tuple_count, u32 level)
   {
      ...
      auto exceptions = get_level_data(exceptions_v, exceptions_bitmap.cardinality() + SIMD_EXTRA_ELEMENTS(NumberType), level);
      if ( exceptions_bitmap.cardinality() > 0 ) {
          CSchemePicker<NumberType, SchemeType, StatsType, SchemeCodeType>::MyTypeWrapper::getScheme(
                  col_struct.next_scheme).decompress(
                  exceptions, nullptr, col_struct.data + col_struct.exceptions_offset,
      ...
      exceptions_bitmap.iterate([](uint32_t value, void *param) {
            // TODO are the values actually being sorted here??? If not this won't work.
...
// -------------------------------------------------------------------------------------
static inline
string fullDescription(const u8 *src, const string& selfDescription) {
    const auto &col_struct = *reinterpret_cast<const FrequencyStructure<NumberType> *> (src);
    auto result = selfDescription;

    Roaring exceptions_bitmap = Roaring::read(reinterpret_cast<const char *>(col_struct.data), false);
    if ( exceptions_bitmap.cardinality() > 0 ) {
        auto &scheme = CSchemePicker<NumberType, SchemeType, StatsType, SchemeCodeType>::MyTypeWrapper::getScheme(col_struct.next_scheme);
        result += " -> ([NumberType] exceptions) " + scheme.fullDescription(col_struct.data + col_struct.exceptions_offset);
    }
