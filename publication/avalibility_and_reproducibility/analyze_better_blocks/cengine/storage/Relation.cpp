...
Relation::Relation()
{
   columns.reserve(100); // Dirty fix: somehow columns Vector get destroyed and never correctly restored during vector resize
}
// -------------------------------------------------------------------------------------
Relation::Relation(const YAML::Node &schema, const string &columns_dir)
{
   ...
   for ( u32 column_i = 0; column_i < columns.size(); column_i++ ) {
      ...
      string column_type = column["type"].as<string>();
      ...
      // -------------------------------------------------------------------------------------
      const string column_file_prefix = columns_dir + std::to_string(column_i + 1) + "_" + column_name;
...
void Relation::addColumn(const string column_file_path)
{
   ...
   if ( std::regex_search(column_file_path, match, re) && match.size() > 1 ) {
      ...
      bitmap_file_path = bitmap_file_path.replace(bitmap_file_path.end() - column_type_str.length(), bitmap_file_path.end(), "bitmap");
...
// -------------------------------------------------------------------------------------
// This could as well just return vector<cengine::Range>
vector<tuple<u64, u64>> Relation::getRanges(cengine::SplitStrategy strategy, u32 max_chunk_count) const
{
   // -------------------------------------------------------------------------------------
   ...
   // -------------------------------------------------------------------------------------
   if ( max_chunk_count ) {
      ranges.resize(std::min(SIZE(max_chunk_count), ranges.size()));
   }
   return ranges;
}
// -------------------------------------------------------------------------------------
Chunk Relation::getChunk(vector<cengine::Range> &ranges, SIZE chunk_i) const
{
   ...
   for ( u32 i = 0; i < columns.size(); i++ ) {
      ...
      std::memcpy(reinterpret_cast<void *>(c_bitmaps[i].get()), columns[i].bitmaps().data + offset, chunk_tuple_count * sizeof(BITMAP));
      // -------------------------------------------------------------------------------------
      switch ( columns[i].type ) {
         case ColumnType::INTEGER: {
            c_sizes[i] = chunk_tuple_count * sizeof(INTEGER);
            c_columns[i] = std::unique_ptr<u8[]>(new u8[c_sizes[i]]);
            std::memcpy(reinterpret_cast<void *>(c_columns[i].get()), columns[i].integers().data + offset, chunk_tuple_count * sizeof(INTEGER));
            break;
         }
         case ColumnType::DOUBLE: {
            c_sizes[i] = chunk_tuple_count * sizeof(DOUBLE);
            c_columns[i] = std::unique_ptr<u8[]>(new u8[c_sizes[i]]);
            std::memcpy(reinterpret_cast<void *>(c_columns[i].get()), columns[i].doubles().data + offset, chunk_tuple_count * sizeof(DOUBLE));
            break;
         }
         case ColumnType::STRING: {
            ...
            // -------------------------------------------------------------------------------------
            // copy the strings
            std::memcpy(reinterpret_cast<void *>(c_columns[i].get() + slots_size), columns[i].strings()[offset].data(), strings_size);
            // -------------------------------------------------------------------------------------
            break;
         }
         default:
...
InputChunk Relation::getInputChunk(Range &range, SIZE chunk_i, u32 column) const {
    ...
    std::memcpy(reinterpret_cast<void *>(bitmap.get()), columns[column].bitmaps().data + offset, chunk_tuple_count * sizeof(BITMAP));
    ...
    switch ( columns[column].type ) {
        case ColumnType::INTEGER: {
            ...
            std::memcpy(reinterpret_cast<void *>(data.get()), columns[column].integers().data + offset, chunk_tuple_count * sizeof(INTEGER));
            break;
        }
        case ColumnType::DOUBLE: {
            size = chunk_tuple_count * sizeof(DOUBLE);
            data = std::unique_ptr<u8[]>(new u8[size]);
            std::memcpy(reinterpret_cast<void *>(data.get()), columns[column].doubles().data + offset, chunk_tuple_count * sizeof(DOUBLE));
            break;
        }
        case ColumnType::STRING: {
            const u64 slots_size = sizeof(StringArrayViewer::Slot) * (chunk_tuple_count + 1);
            ...
            // -------------------------------------------------------------------------------------
            // copy the strings
            std::memcpy(reinterpret_cast<void *>(data.get() + slots_size), columns[column].strings()[offset].data(), strings_size);
            // -------------------------------------------------------------------------------------
            break;
        }
        default:
