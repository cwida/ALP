...
struct StringArrayViewer {
   ...
   StringArrayViewer(const u8 *slots_ptr)
     : slots_ptr(slots_ptr) {}

   inline static const str get(const u8 *slots_ptr, u32 i)
   {
      auto slots = reinterpret_cast<const Slot *>(slots_ptr);
      return str(reinterpret_cast<const char *>(reinterpret_cast<const u8 *>(slots_ptr) + slots[i].offset), slots[i + 1].offset - slots[i].offset);
   }
   ...
   inline const char *get_pointer(u32 i) const
   {
       auto slots = reinterpret_cast<const Slot *>(slots_ptr);
