...
struct InputFiles {
	...
	bool next(std::string& output) { return !(std::getline(list, output).fail()); }
};

std::string ensure_file(const std::string& object) { return object; }

using T = double;
bool test_compression(
    cengine::db::DoubleScheme& scheme, cengine::db::DoubleStats& stats, T* src, size_t size, PerfEvent& e, u8 cascade) {
	...
	e.setParam("phase", "compression");
	{
		...
		e.setParam("compr", output_bytes / (1.0 * size * sizeof(T)) * 64);
	}
...
int main(int argc, char* argv[]) {
	...
	PerfEvent perf;
	std::cerr << "using cascade depth " << FLAGS_cascade_depth << " and input file " << FLAGS_file_list_file
	          << std::endl;
	...
	while (file_list.next(nextfile)) {
...

		{
			...
			//			spdlog::info("size: {:03.2f} MiB, head: {}", (sizeof(T) * doubles.size()) * 1.0 / 1024 / 1024,
			//head);
		}
