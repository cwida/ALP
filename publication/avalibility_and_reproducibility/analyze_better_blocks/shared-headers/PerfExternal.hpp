...
struct PerfExternal {
    static void start(bool print = false) {
        ...
        if (print) { std::cout << "perf point 1 done, read" << input << "/" << input.size() << std::endl; }
    }
...
struct PerfExternalBlock {
    ...
    PerfExternalBlock(bool activate) : activate(activate) { start(); }
    PerfExternalBlock() {
        char* e = getenv("PERF");
        activate = e != nullptr && (
            !strcmp(e, "true") || !strcmp(e, "t") || !strcmp(e, "y") || !strcmp(e, "yes") || !strcmp(e, "1")
        );
        start();
    }
    ...
    private:
        void start() {if (activate) { PerfExternal::start(false); }}
        void stop() {if (activate) { PerfExternal::stop(false); }}
};
