...
BtrReader::~BtrReader() {
    ...
    // (unless we release the bitmap first in which case the wrapper is deleted, but the bitset may still exist)
...
string BtrReader::getSchemeDescription(u32 index) {
    ...
    switch (meta->type) {
        ...
        default: throw Generic_Exception("Type " + ConvertTypeToString(meta->type) + " not supported");
    }
}
...
BitmapWrapper *BtrReader::getBitmap(u32 index) {
    ...
    if (this->m_bitsets[index] == nullptr && type != BitmapType::ALLONES && type != BitmapType::ALLZEROS) {
        this->m_bitsets[index] = new boost::dynamic_bitset<>(meta->tuple_count);
    }
    // TODO if there are too many page fault try to do the allocation of the bitset inside the object beforehand
    this->m_bitmap_wrappers[index] = new BitmapWrapper(
        meta->data + meta->nullmap_offset,
        type,
        meta->tuple_count,
        this->m_bitsets[index]
    );
    return this->m_bitmap_wrappers[index];
}
...
u32 BtrReader::getDecompressedSize(u32 index) {
    ...
    switch (meta->type) {
        ...
        default: {
            throw Generic_Exception("Type " + ConvertTypeToString(this->getColumnType()) + " not supported");
        }
    }
}
