...
namespace db {
// -------------------------------------------------------------------------------------
Datablock::Datablock(const Relation &relation)
  : CMachine(relation)
{
}
u32 Datablock::writeMetadata(const std::string &path, std::vector<ColumnType> types, vector<u32> part_counters,
                             u32 num_chunks) {
...
std::vector<u8> Datablock::compress(const InputChunk &input_chunk) {
    // We do not now the exact output size. Therefore we allocate too much and then simply make the space smaller afterwards
    const u32 size = sizeof(ColumnChunkMeta) + 10 * input_chunk.size +  sizeof(BITMAP) * input_chunk.tuple_count;
    ...
    switch ( input_chunk.type ) {
        case ColumnType::INTEGER: {
            IntegerSchemePicker::compress(reinterpret_cast<INTEGER *>(input_chunk.data.get()), input_chunk.nullmap.get(), output_data, input_chunk.tuple_count, FLAGS_integers_max_cascading_level, meta->nullmap_offset, meta->compression_type);
            break;
        }
        case ColumnType::DOUBLE : {
            // -------------------------------------------------------------------------------------
            DoubleSchemePicker::compress(reinterpret_cast<DOUBLE *>(input_chunk.data.get()), input_chunk.nullmap.get(), output_data, input_chunk.tuple_count, FLAGS_doubles_max_cascading_level, meta->nullmap_offset, meta->compression_type);
            // -------------------------------------------------------------------------------------
            break;
        }
        case ColumnType::STRING : {
            // -------------------------------------------------------------------------------------
            // Collect stats
            StringStats stats = StringStats::generateStats(StringArrayViewer(input_chunk.data.get()), input_chunk.nullmap.get(), input_chunk.tuple_count, input_chunk.size);
            // -------------------------------------------------------------------------------------
            // Make decisions
            StringScheme &preferred_scheme = StringSchemePicker::chooseScheme(stats, FLAGS_strings_max_cascading_level);
            // Update meta data
            ...
            u32 after_column_size = preferred_scheme.compress(str_viewer, input_chunk.nullmap.get(), output_data, stats);
            meta->nullmap_offset = after_column_size;
            // -------------------------------------------------------------------------------------
            for ( u8 i = 0; i < 5 - FLAGS_strings_max_cascading_level; i++ ) {
                ThreadCache::get() << "\t";
            }
            ThreadCache::get() << "for : ? - scheme = " + ConvertSchemeTypeToString(preferred_scheme.schemeType());
            ...
            double estimated_cf = CD(stats.total_size) / CD(after_column_size); // we only have one scheme for strings (beside ONE_VALUE)
            ThreadCache::dumpPush(ConvertSchemeTypeToString(preferred_scheme.schemeType()) + (ThreadCache::hasUsedFsst() ? "_FSST" : ""),
                                  estimated_cf,
                                  stats.total_size, after_column_size,
                                  stats.unique_count, "?");
            ThreadCache::get().compression_level--;
            // -------------------------------------------------------------------------------------
            break;
    ...
    // Compress bitmap
    auto [nullmap_size, bitmap_type] = v2::bitmap::RoaringBitmap::compress(input_chunk.nullmap.get(), output_data + meta->nullmap_offset, input_chunk.tuple_count);
...
bool Datablock::decompress(const u8 *data_in, BitmapWrapper **bitmap_out, u8 *data_out) {
    ...
    // Decompress bitmap
    *bitmap_out = new BitmapWrapper(
            meta->data + meta->nullmap_offset,
            meta->nullmap_type,
            meta->tuple_count
    );

    // Decompress data, we assume that data_out already has the correct size allocated
    bool requires_copy_out;
    switch ( meta->type ) {
        case ColumnType::INTEGER : {
            auto &scheme = CSchemePool::available_schemes->integer_schemes[static_cast<IntegerSchemeType>(meta->compression_type)];
            scheme->decompress(reinterpret_cast<INTEGER *>(data_out), *bitmap_out,
                              meta->data, meta->tuple_count, 0);
            requires_copy_out = false;
            break;
        }
        case ColumnType::DOUBLE : {
            auto &scheme = CSchemePool::available_schemes->double_schemes[static_cast<DoubleSchemeType>(meta->compression_type)];
            scheme->decompress(reinterpret_cast<DOUBLE *>(data_out), *bitmap_out, meta->data, meta->tuple_count, 0);
            requires_copy_out = false;
            break;
        }
        case ColumnType::STRING : {
            auto &scheme = CSchemePool::available_schemes->string_schemes[static_cast<StringSchemeType>(meta->compression_type)];
            requires_copy_out = scheme->decompressNoCopy(data_out, *bitmap_out, meta->data, meta->tuple_count, 0);
            break;
        }
        default:
            throw Generic_Exception("Type not supported");
...
{
   // -------------------------------------------------------------------------------------
   const u32 db_meta_buffer_size = sizeof(DatablockMeta) + (relation.columns.size() * sizeof(ColumnMeta));
   ...
   if ( !output_block ) {
      ...
      const u32 output_block_size = (db_meta_buffer_size + input_chunk_total_data_size) * 10 + (relation.columns.size() * sizeof(BITMAP) * input_chunk.tuple_count);
   ...
   OutputBlockStats output_db_stats = {
     ...
     , .total_db_size = db_meta_buffer_size}; // we are going to determine the size during columns analysis
   ...
   for ( u32 column_i = 0; column_i < relation.columns.size(); column_i++ ) {
      ...
      // -------------------------------------------------------------------------------------
      spdlog::info("DB: compressing column : {}", column.name);
      ...
      switch ( column.type ) {
         case ColumnType::INTEGER: {
            ...
            // -------------------------------------------------------------------------------------
            // Compression
            IntegerSchemePicker::compress(input_chunk.array<INTEGER>(column_i), nullmap, output_block.get() + db_write_offset, input_chunk.tuple_count, FLAGS_integers_max_cascading_level, after_column_size, column_meta.compression_type);
         ...
         case ColumnType::DOUBLE : {
            // -------------------------------------------------------------------------------------
            DoubleSchemePicker::compress(input_chunk.array<DOUBLE>(column_i), input_chunk.nullmap(column_i), output_block.get() + db_write_offset, input_chunk.tuple_count, FLAGS_doubles_max_cascading_level, after_column_size, column_meta.compression_type);
            // -------------------------------------------------------------------------------------
            break;
         }
         case ColumnType::STRING : {
            // -------------------------------------------------------------------------------------
            // Collect stats
            StringStats stats = StringStats::generateStats(StringArrayViewer(input_chunk.array<const u8>(column_i)), input_chunk.nullmap(column_i), input_chunk.tuple_count, input_chunk.size(column_i));
            // -------------------------------------------------------------------------------------
            // Make decisions
            StringScheme &preferred_scheme = StringSchemePicker::chooseScheme(stats, FLAGS_strings_max_cascading_level);
            // Update meta data
            ...
            after_column_size = preferred_scheme.compress(str_viewer, input_chunk.nullmap(column_i), output_block.get() + db_write_offset, stats);
            // -------------------------------------------------------------------------------------
            for ( u8 i = 0; i < 5 - FLAGS_strings_max_cascading_level; i++ ) {
               ThreadCache::get() << "\t";
            }
      ...
      auto [nullmap_size, bitmap_type] = v2::bitmap::RoaringBitmap::compress(input_chunk.nullmap(column_i), output_block.get() + db_write_offset, input_chunk.tuple_count);
      ...
      ThreadCache::get() << "name = " + column.name + " type = " + ConvertTypeToString(column.type)
                            ...
                            + " after = " + std::to_string(after_column_size)
                            + " gain = " + std::to_string(CD(input_chunk.size(column_i)) / CD(after_column_size))
                            + "\n\n\n";
   }
   // -------------------------------------------------------------------------------------
   ...
   output_db_stats.total_db_size += output_db_stats.total_nullmap_size + output_db_stats.total_data_size;
   assert(db_write_offset <= output_db_stats.total_db_size);
   output_db_stats.compression_ratio = static_cast<double>(input_chunk_total_data_size) / static_cast<double >(output_db_stats.total_data_size);
   // -------------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------------
cengine::Chunk Datablock::decompress(const BytesArray &input_db)
{
   ...
   // -------------------------------------------------------------------------------------
   auto columns = std::unique_ptr<std::unique_ptr<u8[]>[]>(new std::unique_ptr<u8[]>[relation.columns.size()]);
   std::unique_ptr<bool[]> column_requires_copy(new bool[relation.columns.size()]);
   auto bitmaps = std::unique_ptr<std::unique_ptr<BITMAP[]>[]>(new std::unique_ptr<BITMAP[]>[relation.columns.size()]);
   ...
   for ( u32 column_i = 0; column_i < relation.columns.size(); column_i++ ) {
      ...
      auto bitmap = BitmapWrapper(reinterpret_cast<const u8 *>(input_db.get() + column_meta.nullmap_offset), column_meta.bitmap_type, tuple_count);
      ...
      switch ( column.type ) {
         case ColumnType::INTEGER : {
            ...
            // -------------------------------------------------------------------------------------
            scheme.decompress(destination_array, &bitmap,
                               input_db.get() + column_meta.offset, tuple_count, 0);
         ...
         case ColumnType::DOUBLE : {
            ...
            const auto used_compression_scheme = static_cast<DoubleSchemeType >(column_meta.compression_type);
            auto &scheme = CSchemePool::available_schemes->double_schemes[used_compression_scheme];
            // -------------------------------------------------------------------------------------
            scheme->decompress(column_dest_double_array, &bitmap, input_db.get() + column_meta.offset, tuple_count, 0);
            column_requires_copy[column_i] = false;
            break;
         }
         case ColumnType::STRING : {
            // -------------------------------------------------------------------------------------
            const auto used_compression_scheme = static_cast<StringSchemeType>(column_meta.compression_type);
            auto &scheme = CSchemePool::available_schemes->string_schemes[used_compression_scheme];
            // -------------------------------------------------------------------------------------
            sizes[column_i] = scheme->getDecompressedSizeNoCopy(
                    input_db.get() + column_meta.offset,
                    tuple_count,
                    &bitmap);
            // TODO The 4096 is temporary until I figure out why FSST is returning bigger numbers
            columns[column_i] = makeBytesArray(sizes[column_i] + 8 + SIMD_EXTRA_BYTES + 4096); // +8 because of 8 fsst decompression
            // -------------------------------------------------------------------------------------
            column_requires_copy[column_i] = scheme->decompressNoCopy(columns[column_i].get(), &bitmap, input_db.get() + column_meta.offset, tuple_count, 0);
            break;
         }
         default:
            throw Generic_Exception("Type not supported");
   ...
   // -------------------------------------------------------------------------------------
   return Chunk(std::move(columns), std::move(bitmaps), std::move(column_requires_copy), tuple_count, relation, std::move(sizes));
}
// -------------------------------------------------------------------------------------
