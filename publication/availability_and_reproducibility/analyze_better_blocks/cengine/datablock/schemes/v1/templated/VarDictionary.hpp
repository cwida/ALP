...
inline double VDictExpectedCompressionRatio(StringStats &stats)
{
   if ( stats.distinct_values.size() > (std::numeric_limits<CodeType>::max())) {
      ...
      u32 after_size = (stats.tuple_count * (sizeof(CodeType))) + (sizeof(StringArrayViewer::Slot) * (1 + stats.distinct_values.size())) + sizeof(VarDictionaryStructure);
...
// -------------------------------------------------------------------------------------
template<typename CodeType>
inline u32 VDictCompressColumn(const StringArrayViewer src, const BITMAP *, u8 *dest, StringStats &stats)
{
   ...
   // -------------------------------------------------------------------------------------
   // set is assumed to be implemented as red-black tree with order preserving iterator
...
// -------------------------------------------------------------------------------------
template<typename CodeType>
inline void VDictDecompressColumn(u8 *dest, BitmapWrapper *, const u8 *src, u32 tuple_count, u32 level)
{
