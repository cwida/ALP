...
public:
   ...
   {
      return IntegerSchemeType::TRUNCATION_16;
   }
   // -------------------------------------------------------------------------------------
   INTEGER lookup(u32);
...
public:
   ...
   inline static IntegerSchemeType staticSchemeType()
   {
      return IntegerSchemeType::TRUNCATION_8;
   }
   // -------------------------------------------------------------------------------------
   INTEGER lookup(u32);
   void scan(Predicate, BITMAP *, const u8 *, u32);
...
// -------------------------------------------------------------------------------------
template<typename CodeType>
double ITruncExpectedCF(cengine::db::SInteger32Stats &stats)
{
...
// -------------------------------------------------------------------------------------
template<typename CodeType>
void ITruncDecompress(INTEGER *dest, BitmapWrapper *nullmap, const u8 *src, u32 tuple_count, u32 level)
{
    /* As of now Truncation is unused and this part is commented out for simpler refactoring */
