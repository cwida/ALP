...
BitmapWrapper::BitmapWrapper(const u8 *src, BitmapType type, u32 tuple_count, boost::dynamic_bitset<> *bitset) : m_tuple_count(tuple_count), m_bitset(bitset), m_type(type) {
...
boost::dynamic_bitset<> *BitmapWrapper::get_bitset() {
    ...
    switch (this->m_type) {
        ...
        case BitmapType::REGULAR: {
            this->m_bitset->reset();
            this->m_roaring.iterate([](uint32_t value, void *param) {
        ...
        default: {
            throw Generic_Exception("Unknown BitmapType " + std::to_string(static_cast<int>(this->m_type)));
        }
    }
...
// -------------------------------------------------------------------------------------
std::pair<u32, BitmapType> RoaringBitmap::compress(const BITMAP *bitmap, u8 *dest, u32 tuple_count)
{
   ...
   if (ones_count < tuple_count/2) {
       ...
       // There are more 1s than 0s in the Bitmap. In oder to save space and computation time during decompression of
       // the roaring bitmap we simply flip the bits. (Less 1s => Smaller Roaring bitmap)
