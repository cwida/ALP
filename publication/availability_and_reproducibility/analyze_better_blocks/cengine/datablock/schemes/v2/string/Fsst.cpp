...
namespace cengine::db::v2::string {
    double Fsst::expectedCompressionRatio(StringStats &stats, u8) {
        // TODO
        return 1.0;
    }

    u32 Fsst::compress(const cengine::StringArrayViewer src, const BITMAP *, u8 *dest, cengine::db::StringStats &stats) {
        // TODO: For now we consider null values to be empty strings. Maybe it would be faster to not compress them
        // and use roaring bitmap iterate to fill the StringArrayView on decompression
        ...
        for (u32 str_i = 0; str_i < stats.tuple_count; str_i++) {
            input_string_buffers[str_i] = const_cast<u8 *>(reinterpret_cast<const u8 *>(src.get_pointer(str_i)));
            input_string_lengths[str_i] = src.size(str_i);
        }

        // Prepare decoder and write header
        fsst_encoder_t *encoder = fsst_create(stats.tuple_count, input_string_lengths.get(), input_string_buffers.get(), 0);
        ...
        if ( fsst_compress(encoder, stats.tuple_count, input_string_lengths.get(), input_string_buffers.get(),
                           output_buffer_size, write_ptr,
                           output_string_lengths.get(), output_string_buffers.get()) != stats.tuple_count ) {
            throw Generic_Exception("FSST Compression failed !");
        }
        u64 fsst_strings_used_space = output_string_lengths[stats.tuple_count - 1] + (output_string_buffers[stats.tuple_count - 1] - output_string_buffers[0]);

        spdlog::debug("FSST: strings_s = {}", static_cast<s64>(fsst_strings_used_space));
        spdlog::debug("FSST: string_size : before = {} after = {}", static_cast<s64>(stats.total_length), static_cast<s64>(fsst_table_used_space + fsst_strings_used_space));
        ...
        u32 used_space;
        IntegerSchemePicker::compress(reinterpret_cast<const INTEGER *>(src.slots_ptr), nullptr, write_ptr, stats.tuple_count+1, 2, used_space, col_struct.offsets_scheme);
    ...
    void Fsst::decompress(u8 *dest, BitmapWrapper *, const u8 *src, u32 tuple_count, u32 level) {
        ...
        IntegerScheme &offsets_scheme = IntegerSchemePicker::MyTypeWrapper::getScheme(col_struct.offsets_scheme);
        offsets_scheme.decompress(reinterpret_cast<INTEGER *>(dest_slots), nullptr, compressed_offsets, tuple_count + 1, level + 1);

        // Decompress strings
        fsst_decoder_t decoder;
        die_if(fsst_import(&decoder, const_cast<u8 *>(col_struct.data)) > 0)
        auto compressed_strings = const_cast<u8 *>(col_struct.data + col_struct.strings_offset);
        auto decompressed_strings_size = col_struct.total_decompressed_size - ((tuple_count+1) * sizeof(INTEGER));
        decompressed_strings_size += 4096;
        dest_write_ptr += fsst_decompress(&decoder, col_struct.compressed_strings_size, compressed_strings, decompressed_strings_size, dest_write_ptr);
    ...
    std::string Fsst::fullDescription(const u8 *src) {
        auto &col_struct = *reinterpret_cast<const FsstStructure *> (src);
        IntegerScheme &offsets_scheme = IntegerSchemePicker::MyTypeWrapper::getScheme(col_struct.offsets_scheme);
        return this->selfDescription(src) + " -> ([int] offsets) " + offsets_scheme.fullDescription(col_struct.data + col_struct.offsets_offset);
    }
